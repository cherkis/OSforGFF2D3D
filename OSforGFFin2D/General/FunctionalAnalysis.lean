...
/-- **2D spatial polynomial decay integrability**: `(1 + ‖x‖)^{-4}` is integrable on ℝ²
    ...
    **Mathematical content**: In ℝ² with polar coordinates,
    ∫ 1/(1+r)^4 · r dr dθ = 2π ∫₀^∞ r/(1+r)^4 dr < ∞
    since the integrand decays as r⁻³ for large r.
...
lemma polynomial_decay_integrable_1d :
    Integrable (fun x : EuclideanSpace ℝ (Fin 1) => 1 / (1 + ‖x‖)^4) volume := by
  have hdim : Module.finrank ℝ (EuclideanSpace ℝ (Fin 1)) = 1 := finrank_euclideanSpace
  have hdim_lt : (Module.finrank ℝ (EuclideanSpace ℝ (Fin 1)) : ℝ) < (4 : ℝ) := by
    rw [hdim]; norm_num
  have h_int := integrable_one_add_norm (E := EuclideanSpace ℝ (Fin 1)) (μ := volume) (r := 4) hdim_lt
  convert h_int using 1
  ext x
  have h_pos : 0 < 1 + ‖x‖ := by linarith [norm_nonneg x]
  simp only [Real.rpow_neg (le_of_lt h_pos), one_div]
...
lemma polynomial_decay_integrable_2d :
    Integrable (fun x : EuclideanSpace ℝ (Fin 2) => 1 / (1 + ‖x‖)^4) volume := by
  -- Use integrable_one_add_norm: (1 + ‖x‖)^(-r) is integrable when r > dim
  have hdim : Module.finrank ℝ (EuclideanSpace ℝ (Fin 2)) = 2 := finrank_euclideanSpace
  have hdim_lt : (Module.finrank ℝ (EuclideanSpace ℝ (Fin 2)) : ℝ) < (4 : ℝ) := by
    rw [hdim]; norm_num
  have h_int := integrable_one_add_norm (E := EuclideanSpace ℝ (Fin 2)) (μ := volume) (r := 4) hdim_lt
  -- Convert (1 + ‖x‖)^(-4) to 1 / (1 + ‖x‖)^4
  convert h_int using 1
  ext x
  have h_pos : 0 < 1 + ‖x‖ := by linarith [norm_nonneg x]
