...
* `besselK0` - The modified Bessel function K₀(z) = ∫₀^∞ exp(-z cosh(t)) dt
...
* `schwingerIntegral_eq_besselK0` - Identity connecting Schwinger integral to K₀
...
K₀(z) = ∫₀^∞ exp(-z cosh(t)) dt is the modified Bessel function of the second kind
of order 0. This appears as the free propagator kernel in 2D Euclidean QFT:
  C(x,y) = (1/(2π)) K₀(m|x-y|)
-/
...
/-! K₀ properties (positivity, continuity, asymptotics, near-origin bound,
    integrability, Schwinger integral) are proved in `OSforGFF.BesselK0Proofs`. -/
...
lemma besselK1_asymptotic (z : ℝ) (hz : 1 ≤ z) :
  ...
  have h_part1 : ∫ t in Icc 0 1, f t ≤ sinh 1 * exp (-z) := by
    ...
    have h_val : ∫ t in Icc 0 1, exp (-z) * cosh t = exp (-z) * sinh 1 := by
      ...
      simp only [sinh_zero, mul_zero, sub_zero, Pi.mul_apply] at h
