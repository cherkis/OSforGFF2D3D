...
In 2D (d=2), this simplifies to:

  C(r) = (1/(4π)) ∫₀^∞ e^{-tm²} · (1/t) · e^{-r²/(4t)} dt

The remaining 1D integral can be computed via the substitution u = m²t + r²/(4t),
which leads to the Bessel K₀ function: C(r) = (1/(2π)) K₀(mr).
-/
...
/-- For d = 2, the heat kernel simplifies to 1/(4πt) · exp(-r²/(4t)). -/
lemma heatKernelPositionSpace_2D (t : ℝ) (ht : 0 < t) (r : ℝ) :
    heatKernelPositionSpace t r = 1 / (4 * Real.pi * t) * Real.exp (-r^2 / (4 * t)) := by
  unfold heatKernelPositionSpace
  have hd : (STDimension : ℝ) = 2 := by simp [STDimension]
  rw [hd]
  -- (4πt)^{-1} = 1/(4πt)
  have hpos : 0 < 4 * Real.pi * t := by positivity
  have h1 : (4 * Real.pi * t) ^ (-(2 : ℝ) / 2) = 1 / (4 * Real.pi * t) := by
...
lemma heatKernelPositionSpace_bounded (r : ℝ) (hr : 0 < r) :
  ...
  -- For d=2: H(s,r) = (4πs)^{-1} * exp(-r²/(4s))
  -- Use the bound: u * exp(-cu) ≤ 1/c (maximum at u = 1/c)
  -- With u = 1/s and c = r²/4: H = (4π)⁻¹ * u * exp(-r²u/4) ≤ (4π)⁻¹ * 4/r²
  use 1 / (Real.pi * r^2) + 1
  constructor
  · positivity
  · intro s hs
    have hst_dim : (STDimension : ℝ) = 2 := by simp [STDimension]
    unfold heatKernelPositionSpace
    rw [hst_dim]
    -- For d=2: (4πs)^{-1} * exp(-r²/(4s))
    -- Bound: s^{-1} * exp(-r²/(4s)) ≤ 4/r² (maximum of u*exp(-cu) = 1/(ce) at u=1/c)
    -- So H ≤ (4π)⁻¹ * 4/r² = 1/(πr²)
    -- Key helper: u * exp(-u) ≤ 1 (since u ≤ u+1 ≤ exp(u))
    have mul_exp_neg_le : ∀ u : ℝ, u * Real.exp (-u) ≤ 1 := by
      intro u
      rw [show u * Real.exp (-u) = u / Real.exp u by
        rw [Real.exp_neg, div_eq_mul_inv]]
      rw [div_le_one (Real.exp_pos u)]
      linarith [Real.add_one_le_exp u]
    have h4ps : 0 < 4 * Real.pi * s := by positivity
    have h_rpow : (4 * Real.pi * s) ^ (-(2 : ℝ) / 2) = 1 / (4 * Real.pi * s) := by
      rw [show -(2 : ℝ) / 2 = -1 by norm_num]
      rw [Real.rpow_neg (le_of_lt h4ps), Real.rpow_one]; ring
    rw [h_rpow]
    have hpr : 0 < Real.pi * r ^ 2 := by positivity
    have h_eq : 1 / (4 * Real.pi * s) * Real.exp (-r ^ 2 / (4 * s)) =
        (r ^ 2 / (4 * s)) * Real.exp (-(r ^ 2 / (4 * s))) / (Real.pi * r ^ 2) := by
      field_simp
    rw [h_eq]
    linarith [div_le_div_of_nonneg_right (mul_exp_neg_le (r ^ 2 / (4 * s))) (le_of_lt hpr)]
...
theorem heatKernelPositionSpace_integral_eq_one (t : ℝ) (ht : 0 < t) :
  ...
  have hd_real : (STDimension : ℝ) = 2 := by simp [STDimension]
  have h_finrank : Module.finrank ℝ (SpaceTime) = 2 := finrank_euclideanSpace_fin
  ...
  -- Now: (4πt)^{-1} × (π / (1/(4t)))^1 = (4πt)^{-1} × (4πt)^1 = 1
  rw [hd_real]
  have h_div_eq : π / (1 / (4 * t)) = 4 * π * t := by field_simp
  rw [h_div_eq]
  have h_pow_eq : (4 * π * t) ^ ((2 : ℝ) / 2) = (4 * π * t) ^ (1 : ℝ) := by
    congr 1; norm_num
  rw [h_pow_eq]
  have h_neg_pow : (4 * π * t) ^ (-(2 : ℝ) / 2) = (4 * π * t) ^ (-(1 : ℝ)) := by
    congr 1; norm_num
  rw [h_neg_pow]
  ...
  rw [Real.rpow_one]
  -- Goal: (4πt)⁻¹ * (4πt) = 1
  have h_ne : (4 * π * t) ≠ 0 := ne_of_gt h_pos
...
/-- In 2D, the Schwinger representation of the covariance equals:
    (1/(4π)) ∫₀^∞ exp(-tm²) · (1/t) · exp(-r²/(4t)) dt -/
lemma covarianceSchwingerRep_2D (m : ℝ) (_hm : 0 < m) (r : ℝ) (_hr : 0 < r) :
    covarianceSchwingerRep m r =
    (1 / (4 * Real.pi)) * ∫ t in Set.Ioi 0,
      Real.exp (-t * m^2) * (1 / t) * Real.exp (-r^2 / (4 * t)) := by
  unfold covarianceSchwingerRep
  -- Pull out the constant and use heatKernelPositionSpace_2D
  have h : ∀ t ∈ Set.Ioi 0,
      Real.exp (-t * m^2) * heatKernelPositionSpace t r =
      (1 / (4 * Real.pi)) * (Real.exp (-t * m^2) * (1 / t) * Real.exp (-r^2 / (4 * t))) := by
    intro t ht
    rw [heatKernelPositionSpace_2D t ht r]
    ring
  rw [setIntegral_congr_fun measurableSet_Ioi h]
  rw [MeasureTheory.integral_const_mul]

/-- The Schwinger representation of the covariance equals the Bessel formula.
    C(r) = covarianceSchwingerRep m r = (1/(2π)) K₀(mr)

    This is the main result connecting the Schwinger proper-time representation
    to the explicit Bessel function formula for the free scalar propagator in 2D. -/
theorem covarianceSchwingerRep_eq_besselFormula (m r : ℝ) (hm : 0 < m) (hr : 0 < r) :
    covarianceSchwingerRep m r = (1 / (2 * Real.pi)) * besselK0 (m * r) := by
  rw [covarianceSchwingerRep_2D m hm r hr]
  -- We have: (1/(4π)) ∫₀^∞ exp(-tm²) (1/t) exp(-r²/(4t)) dt
  -- This equals (1/(4π)) * 2 * K₀(mr) = (1/(2π)) * K₀(mr)
  have h_integral := schwingerIntegral_eq_besselK0 m r hm hr
  -- First show the integrands are equal
  have h_eq : ∫ t in Set.Ioi 0, Real.exp (-t * m^2) * (1 / t) * Real.exp (-r^2 / (4 * t)) =
              ∫ t in Set.Ioi 0, (1 / t) * Real.exp (-m^2 * t - r^2 / (4 * t)) := by
    apply setIntegral_congr_fun measurableSet_Ioi
    intro t _ht
    simp only []
    have h1 : Real.exp (-t * m^2) * (1 / t) * Real.exp (-r^2 / (4 * t)) =
              (1 / t) * (Real.exp (-t * m^2) * Real.exp (-r^2 / (4 * t))) := by ring
    rw [h1, ← Real.exp_add]
    ring_nf
  rw [h_eq, h_integral]
  ring
...
/-- The free covariance in position space via Bessel function representation.
    ...
    This is the explicit formula for the massive scalar field propagator in 2D.
...
lemma freeCovarianceBessel_pos (m : ℝ) (hm : 0 < m) (x y : SpaceTime) (hxy : x ≠ y) :
  ...
  · apply div_pos one_pos
    ...
    · exact Real.pi_pos
  · exact besselK0_pos (m * ‖x - y‖) (mul_pos hm (norm_pos_iff.mpr (sub_ne_zero.mpr hxy)))
...
4. **Bessel connection**: By `covarianceSchwingerRep_eq_besselFormula`, this equals
   (1/(2π)) K₀(mr) = freeCovarianceBessel m x y
-/
...
lemma gaussianFT_eq_heatKernel_times_norm (s : ℝ) (hs : 0 < s) (z : SpaceTime) :
  ...
  -- For d=2: (π/s)^{d/2} = π/s, and (2π)^2 * H(s,r) = (2π)^2 * (1/(4πs)) * exp(-r²/(4s))
  --         = (π/s) * exp(-r²/(4s)), matching the Gaussian FT result.
  rw [heatKernelPositionSpace_2D s hs ‖z‖]
  have h_finrank : Module.finrank ℝ (SpaceTime) = 2 := finrank_euclideanSpace_fin
  rw [h_finrank]
  -- Algebraic identity: (π/s)^1 * exp(-‖z‖²/(4s)) = (2π)² * (1/(4πs) * exp(-‖z‖²/(4s)))
  have h1 : (↑(2 : ℕ) : ℂ) / 2 = 1 := by norm_num
  rw [h1, cpow_one, Complex.ofReal_mul, Complex.ofReal_exp]
  push_cast
  have hs' : (s : ℂ) ≠ 0 := Complex.ofReal_ne_zero.mpr (ne_of_gt hs)
...
theorem fubini_schwinger_integrand (α : ℝ) (hα : 0 < α) (m : ℝ) (hm : 0 < m)
  ...
  have h_lhs : (∫ k : SpaceTime, (↑(∫ t in Set.Ioi 0,
    ...
    have hmul :=
      (MeasureTheory.integral_const_mul (L := ℂ)
        (Complex.exp (-Complex.I * ⟪k, x - y⟫_ℝ))
        (fun t : ℝ => (↑(Real.exp (-(α + t) * ‖k‖^2) * Real.exp (-t * m^2)) : ℂ))
        (μ := volume.restrict (Set.Ioi 0))).symm
    have hmul' :
        Complex.exp (-Complex.I * ⟪k, x - y⟫_ℝ) *
          (∫ x : ℝ in Set.Ioi 0, (↑(Real.exp (-(α + x) * ‖k‖^2) * Real.exp (-x * m^2)) : ℂ)) =
        ∫ t : ℝ in Set.Ioi 0, Complex.exp (-Complex.I * ⟪k, x - y⟫_ℝ) *
          (↑(Real.exp (-(α + t) * ‖k‖^2) * Real.exp (-t * m^2)) : ℂ) := by
      simpa using hmul
    rw [mul_comm, hmul']
  ...
  have h_step3 : (∫ k : SpaceTime,
      ...
      ((1 / normalisation : ℝ) * ∫ k : SpaceTime,
        ↑(∫ t in Set.Ioi 0, Real.exp (-(α + t) * ‖k‖^2) * Real.exp (-t * m^2)) *
          Complex.exp (-Complex.I * ⟪k, x - y⟫_ℝ)).re := by
    congr 1
    have hfactor : ∀ k : SpaceTime,
        (↑((∫ t in Set.Ioi 0, Real.exp (-(α + t) * ‖k‖^2) * Real.exp (-t * m^2)) / normalisation) : ℂ) *
          Complex.exp (-Complex.I * ⟪k, x - y⟫_ℝ) =
        ↑(1 / normalisation : ℝ) * (↑(∫ t in Set.Ioi 0, Real.exp (-(α + t) * ‖k‖^2) * Real.exp (-t * m^2)) *
          Complex.exp (-Complex.I * ⟪k, x - y⟫_ℝ)) := fun k => by
      simp only [Complex.ofReal_div, Complex.ofReal_one]; ring
    simp_rw [hfactor]
    exact MeasureTheory.integral_const_mul _ _
...
/-- **The deep result:** The regulated Fourier integral converges to the Bessel form as α → 0⁺.
    This is the statement that the Fourier transform of 1/(k² + m²) in 2D equals
    (1 / (2π)) · K₀(mr).

    The proof involves:
    1. Reducing to a 1D integral by exploiting rotational symmetry
    2. Computing the Fourier transform of a radial function in 2D
...
lemma aestronglyMeasurable_freeCovariance (m : ℝ) [Fact (0 < m)] :
  ...
  have hcont : ContinuousOn (fun p : SpaceTime × SpaceTime => (freeCovariance m p.1 p.2 : ℂ)) S := by
    ...
    -- freeCovariance m p.1 p.2 = (1/(2π)) * K₀(m‖p.1-p.2‖) on off-diagonal
    -- This factors as g ∘ (‖fst - snd‖) where g(r) = (1/(2π)) * K₀(mr)
    have h_norm_cont : Continuous (fun p : SpaceTime × SpaceTime => ‖p.1 - p.2‖) :=
      continuous_norm.comp (continuous_fst.sub continuous_snd)
    have h_formula_cont : ContinuousOn (fun r : ℝ => (1 / (2 * Real.pi)) * besselK0 (m * r))
        (Set.Ioi 0) := by
      apply ContinuousOn.mul continuousOn_const
      · apply besselK0_continuousOn.comp (continuousOn_const.mul continuousOn_id)
    ...
    have h_eq : Set.EqOn (fun p => freeCovariance m p.1 p.2)
        (fun p => (1 / (2 * Real.pi)) * besselK0 (m * ‖p.1 - p.2‖)) S := by
      intro p hp
    ...
    have h_comp_cont : ContinuousOn
        (fun p : SpaceTime × SpaceTime => (1 / (2 * Real.pi)) * besselK0 (m * ‖p.1 - p.2‖)) S := by
...
/-- Helper: r * K₀(mr) is integrable on (0, 1] for m > 0.
    Bounded on (0, 1] since r → 0 kills the log singularity of K₀. -/
private lemma rK0_integrableOn_Ioc (m : ℝ) (hm : 0 < m) :
    IntegrableOn (fun r => r * besselK0 (m * r)) (Set.Ioc 0 1) volume := by
  -- The function is continuous on (0,1] and bounded there
  have hcont : ContinuousOn (fun r => r * besselK0 (m * r)) (Set.Ioi 0) := by
    apply ContinuousOn.mul continuousOn_id
    exact besselK0_continuousOn.comp (continuousOn_const.mul continuousOn_id)
      (fun r hr => by simp only [Set.mem_Ioi] at hr ⊢; positivity)
  -- AEStronglyMeasurable
  have hmeas : AEStronglyMeasurable (fun r => r * besselK0 (m * r))
      (volume.restrict (Set.Ioc 0 1)) :=
    (hcont.mono (fun x hx => Set.Ioc_subset_Ioi_self hx)).aestronglyMeasurable measurableSet_Ioc
  -- Bound: on (0, 1], we split into mr ≤ 1 and mr > 1
  -- For mr ≤ 1: r * K₀(mr) ≤ r * (|log(mr/2)| + 1). Since mr/2 ≤ 1/2:
  --   |log(mr/2)| = -log(mr/2) ≤ -log r + |log m| + log 2
  --   r * (-log r) ≤ 1/e (max of x*(-log x) on (0,1])
  --   r * (|log m| + log 2 + 1) ≤ |log m| + log 2 + 1
  --   So r * K₀(mr) ≤ 1/e + |log m| + log 2 + 1
  -- For mr > 1: K₀(mr) ≤ √(π/(2mr)) * exp(-mr) ≤ √(π/2) (using mr ≥ 1, exp ≤ 1)
  --   r * K₀(mr) ≤ 1 * √(π/2) = √(π/2)
  set B := max (1 + |Real.log m| + Real.log 2 + 1) (Real.sqrt (Real.pi / 2)) with hB_def
  have hB_pos : 0 < B := lt_max_of_lt_left (by positivity)
  have hBint : IntegrableOn (fun _ : ℝ => B) (Set.Ioc 0 1) volume :=
    integrableOn_const (hs := measure_Ioc_lt_top.ne)
  apply Integrable.mono' hBint hmeas
  filter_upwards [ae_restrict_mem measurableSet_Ioc] with r hr
  simp only [Set.mem_Ioc] at hr
  have hr_pos : 0 < r := hr.1
  have hmr_pos : 0 < m * r := by positivity
  rw [Real.norm_eq_abs, abs_of_nonneg (mul_nonneg hr_pos.le (besselK0_pos _ hmr_pos).le)]
  rcases le_or_gt (m * r) 1 with hmr_small | hmr_large
  · -- Case mr ≤ 1
    have hbound := besselK0_near_origin_bound (m * r) hmr_pos hmr_small
    calc r * besselK0 (m * r)
        ≤ r * (|Real.log (m * r / 2)| + 1) :=
          mul_le_mul_of_nonneg_left hbound hr_pos.le
      _ ≤ r * (-Real.log r + (|Real.log m| + Real.log 2 + 1)) := by
          apply mul_le_mul_of_nonneg_left _ hr_pos.le
          have hlog_neg : Real.log (m * r / 2) ≤ 0 := Real.log_nonpos (by positivity) (by linarith)
          rw [abs_of_nonpos hlog_neg, Real.log_div (by positivity) (by norm_num),
              Real.log_mul (ne_of_gt hm) (ne_of_gt hr_pos)]
          have : |Real.log m| ≥ -Real.log m := neg_le_abs _
          linarith [Real.log_pos (show (1:ℝ) < 2 by norm_num)]
      _ ≤ 1 + |Real.log m| + Real.log 2 + 1 := by
          -- Key: r * (-log r) ≤ 1 from t * exp(-t) ≤ 1 with t = -log r
          have h_rlogr : r * (-Real.log r) ≤ 1 := by
            have h_neg_log_nonneg : 0 ≤ -Real.log r :=
              neg_nonneg.mpr (Real.log_nonpos hr_pos.le hr.2)
            -- (-log r) * exp(log r) = (-log r) * r ≤ exp(-log r) * exp(log r) = 1
            have key : (-Real.log r) * Real.exp (Real.log r) ≤ 1 := by
              calc (-Real.log r) * Real.exp (Real.log r)
                  ≤ Real.exp (-Real.log r) * Real.exp (Real.log r) := by
                    apply mul_le_mul_of_nonneg_right _ (Real.exp_pos _).le
                    linarith [Real.add_one_le_exp (-Real.log r)]
                _ = 1 := by rw [← Real.exp_add]; simp
            rwa [Real.exp_log hr_pos, mul_comm] at key
          nlinarith [mul_add r (-Real.log r) (|Real.log m| + Real.log 2 + 1),
                    mul_le_of_le_one_left (by positivity : 0 ≤ |Real.log m| + Real.log 2 + 1) hr.2]
      _ ≤ B := le_max_left _ _
  · -- Case mr > 1
    have hbound := besselK0_asymptotic (m * r) hmr_pos
    calc r * besselK0 (m * r)
        ≤ r * (Real.sqrt (Real.pi / (2 * (m * r))) * Real.exp (-(m * r))) :=
          mul_le_mul_of_nonneg_left hbound hr_pos.le
      _ ≤ 1 * (Real.sqrt (Real.pi / 2) * 1) := by
          apply mul_le_mul hr.2 _ (by positivity) (by linarith)
          apply mul_le_mul
          · exact Real.sqrt_le_sqrt (div_le_div_of_nonneg_left (by positivity) (by positivity)
              (by nlinarith))
          · exact Real.exp_le_one_iff.mpr (by nlinarith)
          · exact (Real.exp_pos _).le
          · exact Real.sqrt_nonneg _
      _ = Real.sqrt (Real.pi / 2) := by ring
      _ ≤ B := le_max_right _ _

/-- Helper: r * K₀(mr) is integrable on (1, ∞) for m > 0.
    Exponential decay: K₀(z) ≤ √(π/(2z)) * exp(-z), combined with √(π/(2mr)) ≤ √(π/(2m))
    for r ≥ 1, gives r * K₀(mr) ≤ √(π/(2m)) * r * exp(-mr). -/
private lemma rK0_integrableOn_Ioi (m : ℝ) (hm : 0 < m) :
    IntegrableOn (fun r => r * besselK0 (m * r)) (Set.Ioi 1) volume := by
  -- Dominate by √(π/(2m)) * r * exp(-mr), integrable by rpow_mul_exp_neg result
  have hint : IntegrableOn (fun r => r * Real.exp (-m * r)) (Set.Ioi 0) volume := by
    have := integrableOn_rpow_mul_exp_neg_mul_rpow (p := 1) (s := 1) (b := m)
      (by norm_num : (-1 : ℝ) < 1) le_rfl hm
    simp only [rpow_one] at this; exact this
  apply Integrable.mono' ((hint.mono_set (Set.Ioi_subset_Ioi (show (0:ℝ) ≤ 1 by norm_num))).const_mul
      (Real.sqrt (Real.pi / (2 * m))))
  · apply (ContinuousOn.mul continuousOn_id
      (besselK0_continuousOn.comp (continuousOn_const.mul continuousOn_id)
        (fun r hr => by simp only [Set.mem_Ioi] at hr ⊢; exact mul_pos hm (lt_trans one_pos hr)))).aestronglyMeasurable
      measurableSet_Ioi
  · filter_upwards [ae_restrict_mem measurableSet_Ioi] with r hr
    ...
    calc r * besselK0 (m * r)
        ≤ r * (Real.sqrt (Real.pi / (2 * (m * r))) * Real.exp (-(m * r))) :=
          mul_le_mul_of_nonneg_left (besselK0_asymptotic _ hmr_pos) hr_pos.le
      _ ≤ r * (Real.sqrt (Real.pi / (2 * m)) * Real.exp (-(m * r))) := by
          apply mul_le_mul_of_nonneg_left _ hr_pos.le
          exact mul_le_mul_of_nonneg_right
            (Real.sqrt_le_sqrt (div_le_div_of_nonneg_left (by positivity) (by positivity)
              (by nlinarith)))
            (Real.exp_pos _).le
      _ = Real.sqrt (Real.pi / (2 * m)) * (r * Real.exp (-m * r)) := by ring_nf

/-- **The Bessel covariance kernel is L¹ (integrable on SpaceTime).**

    In d=2 dimensions with kernel K(z) = (1/(2π)) K₀(m‖z‖):
      ∫_{ℝ²} |K(z)| dz = (1/(2π)) · 2π · ∫₀^∞ r K₀(mr) dr < ∞

    The 1D integral is finite because:
    - Near 0: r K₀(mr) is bounded (r → 0 kills the log singularity)
    - At ∞: K₀(mr) decays exponentially

    Proof uses `integrable_fun_norm_addHaar` to reduce to 1D via polar coordinates. -/
theorem freeCovarianceKernel_integrable (m : ℝ) (hm : 0 < m) :
    Integrable (freeCovarianceKernel m) volume := by
  -- Key 1D result: r * K₀(mr) is integrable on (0,∞)
  have hrK0 : IntegrableOn (fun r => r * besselK0 (m * r)) (Set.Ioi 0) volume := by
    have : Set.Ioi (0:ℝ) = Set.Ioc 0 1 ∪ Set.Ioi 1 := by
      ext x; simp only [Set.mem_Ioi, Set.mem_union, Set.mem_Ioc, Set.mem_Ioi]
      exact ⟨fun hx => by rcases le_or_gt x 1 with h | h
                          · exact Or.inl ⟨hx, h⟩
                          · exact Or.inr h,
             fun hx => by rcases hx with ⟨h, -⟩ | h; exact h; linarith⟩
    rw [this]
    exact IntegrableOn.union (rK0_integrableOn_Ioc m hm) (rK0_integrableOn_Ioi m hm)
  -- Express kernel as radial function g(‖z‖) and apply polar decomposition
  set g : ℝ → ℝ := fun r => if r = 0 then 0 else 1 / (2 * Real.pi) * besselK0 (m * r)
  -- Step 1: freeCovarianceKernel m z = g ‖z‖
  have hfg : freeCovarianceKernel m = fun z => g ‖z‖ := by
    ext z; simp only [freeCovarianceKernel, freeCovariance, freeCovarianceBessel, zero_sub,
      norm_neg, g]
  rw [hfg, integrable_fun_norm_addHaar volume (f := g)]
  -- Step 2: RHS is IntegrableOn (fun r => r ^ (d-1) • g r) (Ioi 0) with d = 2
  -- Simplify: d - 1 = 1, r^1 • g(r) = r * (1/(2π)) * K₀(mr) = (1/(2π)) * (r * K₀(mr))
  have hdim : Module.finrank ℝ (SpaceTime) - 1 = 1 := by
    simp [SpaceTime, STDimension]
  -- Step 3: Show equivalent to constant * (r * K₀(mr))
  apply (hrK0.const_mul (1 / (2 * Real.pi))).congr
  filter_upwards [ae_restrict_mem measurableSet_Ioi] with r hr
  simp only [Set.mem_Ioi] at hr
  rw [hdim, pow_one, smul_eq_mul]
  simp only [g, ne_of_gt hr, ↓reduceIte]
  ring

/-- **Logarithmic decay bound for the free covariance kernel in d=2.**

    The free covariance kernel C(z) = (1/(2π)) K₀(m‖z‖) has a logarithmic singularity
    at the origin: K₀(z) ~ -log(z/2) - γ for small z.

    For d=2, there is no power-law singularity (unlike d=4 where C ~ 1/r²).
    Instead, |C(z)| ≤ C₁ * (|log(m‖z‖)| + 1) for ‖z‖ small, and
    |C(z)| ≤ C₂ * exp(-m‖z‖) for ‖z‖ large.

    This combined bound ensures local integrability in 2D. -/
theorem freeCovarianceKernel_decay_bound (m : ℝ) (hm : 0 < m) :
    ∃ C : ℝ, C > 0 ∧ ∀ z : SpaceTime, |freeCovarianceKernel m z| ≤ C * (|Real.log (m * ‖z‖)| + 1) := by
  use 1; refine ⟨one_pos, fun z => ?_⟩; simp only [one_mul]
  by_cases hz : z = 0
  · subst hz; unfold freeCovarianceKernel freeCovariance freeCovarianceBessel
    simp only [zero_sub, norm_neg, norm_zero, mul_zero, ite_true, abs_zero]
    positivity
  · have hz_ne : ‖z‖ ≠ 0 := norm_ne_zero_iff.mpr hz
    have hz_pos : 0 < ‖z‖ := lt_of_le_of_ne (norm_nonneg _) (Ne.symm hz_ne)
    have hmr_pos : 0 < m * ‖z‖ := mul_pos hm hz_pos
    unfold freeCovarianceKernel freeCovariance freeCovarianceBessel
    simp only [zero_sub, norm_neg, hz_ne, ↓reduceIte]
    have hK_pos : 0 < besselK0 (m * ‖z‖) := besselK0_pos _ hmr_pos
    rw [abs_of_pos (mul_pos (by positivity : 0 < 1 / (2 * Real.pi)) hK_pos)]
    -- Suffices to show K₀(ζ) ≤ 2(|log ζ| + 1), since 1/(2π) ≤ 1/2
    suffices h : besselK0 (m * ‖z‖) ≤ 2 * (|Real.log (m * ‖z‖)| + 1) by
      have hpi : 1 / (2 * Real.pi) ≤ 1 / 2 :=
        div_le_div_of_nonneg_left (by norm_num) (by norm_num) (by linarith [Real.two_le_pi])
      linarith [mul_le_mul_of_nonneg_right hpi hK_pos.le,
                mul_le_mul_of_nonneg_left h (show (0:ℝ) ≤ 1/2 by norm_num)]
    rcases le_or_gt (m * ‖z‖) 1 with hmr_small | hmr_large
    · -- Near origin: use besselK0_near_origin_bound
      have hbound := besselK0_near_origin_bound (m * ‖z‖) hmr_pos hmr_small
      have hlog_neg : Real.log (m * ‖z‖) ≤ 0 := Real.log_nonpos hmr_pos.le hmr_small
      have hlogmr2_neg : Real.log (m * ‖z‖ / 2) ≤ 0 := by
        rw [Real.log_div (ne_of_gt hmr_pos) (by norm_num)]
        linarith [Real.log_pos (show (1 : ℝ) < 2 by norm_num)]
      rw [abs_of_nonpos hlogmr2_neg,
          Real.log_div (ne_of_gt hmr_pos) (by norm_num)] at hbound
      rw [abs_of_nonpos hlog_neg]
      -- Need: -log ζ + log 2 + 1 ≤ 2(-log ζ + 1), i.e., log 2 ≤ -log ζ + 1
      have : Real.log 2 < 1 := by
        calc Real.log 2 < Real.log (Real.exp 1) :=
              Real.log_lt_log (by norm_num) (by linarith [Real.exp_one_gt_d9])
          _ = 1 := Real.log_exp 1
      linarith
    · -- Large distance: use asymptotic bound
      have hbound := besselK0_asymptotic (m * ‖z‖) hmr_pos
      have hexp_le : Real.exp (-(m * ‖z‖)) ≤ 1 := Real.exp_le_one_iff.mpr (by linarith)
      have hsqrt_le : Real.sqrt (Real.pi / 2) ≤ 2 := by
        rw [Real.sqrt_le_left (by norm_num : (0:ℝ) ≤ 2)]
        nlinarith [Real.pi_le_four]
      calc besselK0 (m * ‖z‖)
          ≤ Real.sqrt (Real.pi / (2 * (m * ‖z‖))) * Real.exp (-(m * ‖z‖)) := hbound
        _ ≤ Real.sqrt (Real.pi / 2) * 1 := by
            exact mul_le_mul
              (Real.sqrt_le_sqrt (div_le_div_of_nonneg_left
                (by linarith [Real.pi_pos]) (by positivity) (by linarith)))
              hexp_le (Real.exp_nonneg _) (Real.sqrt_nonneg _)
        _ ≤ 2 := by linarith
        _ ≤ 2 * (|Real.log (m * ‖z‖)| + 1) := by
            nlinarith [abs_nonneg (Real.log (m * ‖z‖))]

/-- **Exponential decay bound for the free covariance in d=2.**

    For m > 0 and u, v ∈ ℝ² with m‖u - v‖ ≥ 1:
      |C(u, v)| ≤ C_exp · e^{-m‖u-v‖}

    This combines:
    - The covariance formula: C(u,v) = (1/(2π)) · K₀(m‖u-v‖)
    - The Bessel asymptotic: K₀(z) ≤ √(π/(2z)) · e^{-z} for z > 0 -/
theorem freeCovariance_exponential_bound (m : ℝ) (hm : 0 < m) (u v : SpaceTime)
    (h_sep : 1 ≤ m * ‖u - v‖) :
    |freeCovariance m u v| ≤ (1 / (2 * Real.pi)) * 2 * Real.sqrt (Real.pi / (2 * (m * ‖u - v‖))) * Real.exp (-m * ‖u - v‖) := by
  have hmr_pos : 0 < m * ‖u - v‖ := by linarith
  have hnorm_ne : ‖u - v‖ ≠ 0 := by
    intro h; simp [h] at hmr_pos
  unfold freeCovariance freeCovarianceBessel
  simp only [hnorm_ne, ↓reduceIte]
  have hK_pos : 0 < besselK0 (m * ‖u - v‖) := besselK0_pos _ hmr_pos
  rw [abs_of_pos (mul_pos (by positivity : 0 < 1 / (2 * Real.pi)) hK_pos)]
  have hK_bound := besselK0_asymptotic (m * ‖u - v‖) hmr_pos
  calc 1 / (2 * Real.pi) * besselK0 (m * ‖u - v‖)
      ≤ 1 / (2 * Real.pi) * (Real.sqrt (Real.pi / (2 * (m * ‖u - v‖))) * Real.exp (-(m * ‖u - v‖))) := by
        apply mul_le_mul_of_nonneg_left hK_bound (by positivity)
    _ ≤ 1 / (2 * Real.pi) * 2 * Real.sqrt (Real.pi / (2 * (m * ‖u - v‖))) * Real.exp (-m * ‖u - v‖) := by
        have h_nn : 0 ≤ 1 / (2 * Real.pi) * (Real.sqrt (Real.pi / (2 * (m * ‖u - v‖))) *
          Real.exp (-(m * ‖u - v‖))) := by positivity
        ring_nf at h_nn ⊢
        linarith
...
lemma freeCovariance_exponential_bound' (m : ℝ) [Fact (0 < m)] (u v : SpaceTime)
    (h_sep : 1 ≤ m * ‖u - v‖) :
    |freeCovariance m u v| ≤ (1 / (2 * Real.pi)) * 2 * Real.sqrt (Real.pi / (2 * (m * ‖u - v‖))) * Real.exp (-m * ‖u - v‖) :=
  freeCovariance_exponential_bound m Fact.out u v h_sep

/-- Helper: exp(x) ≥ x²/4 for x ≥ 0, from (exp(x/2))² ≥ (1+x/2)² ≥ (x/2)². -/
private lemma exp_ge_sq_div_four {x : ℝ} (hx : 0 ≤ x) : x ^ 2 / 4 ≤ Real.exp x := by
  have h1 := Real.add_one_le_exp (x / 2)
  have h2 : x / 2 ≤ x / 2 + 1 := le_add_of_nonneg_right one_pos.le
  have h3 : 0 ≤ Real.exp (x / 2) := (Real.exp_pos _).le
  have h4 : x / 2 ≤ Real.exp (x / 2) := by linarith
  have h5 : (x / 2) ^ 2 ≤ Real.exp (x / 2) ^ 2 := sq_le_sq' (by linarith) h4
  rw [sq (Real.exp (x / 2)), ← Real.exp_add] at h5; ring_nf at h5 ⊢; linarith

/-- Helper: exp(-x) ≤ 4/x² for x > 0. -/
private lemma exp_neg_le_four_div_sq {x : ℝ} (hx : 0 < x) :
    Real.exp (-x) ≤ 4 / x ^ 2 := by
  rw [le_div_iff₀ (pow_pos hx 2)]
  have := exp_ge_sq_div_four hx.le
  rw [Real.exp_neg, inv_mul_le_iff₀ (Real.exp_pos x)]
  linarith

/-- **Polynomial decay bound** for the free covariance kernel in d=2.
    K₀(mr) decays exponentially (DLMF §10.25.3), dominating any polynomial 1/r^α.
    Thus ∃ C > 0 such that |K(z)| ≤ C / ‖z‖² for ‖z‖ ≥ 1.
    Proved from besselK0_asymptotic using exp(-x) ≤ 4/x². -/
theorem freeCovarianceKernel_polynomial_bound (m : ℝ) (hm : 0 < m) :
    ∃ C > 0, ∀ z : SpaceTime, ‖z‖ ≥ 1 →
      |freeCovarianceKernel m z| ≤ C / ‖z‖ ^ (2 : ℝ) := by
  -- Use C = 2√(π/(2m)) / (π·m²)
  use 2 * Real.sqrt (Real.pi / (2 * m)) / (Real.pi * m ^ 2)
  refine ⟨by positivity, fun z hz => ?_⟩
  have hz_ne : ‖z‖ ≠ 0 := by linarith
  ...
  have hK_pos : 0 < besselK0 (m * ‖z‖) := besselK0_pos _ hmr_pos
  rw [abs_of_pos (mul_pos (by positivity : 0 < 1 / (2 * Real.pi)) hK_pos)]
  have hK_bound := besselK0_asymptotic (m * ‖z‖) hmr_pos
  -- √(π/(2mr)) ≤ √(π/(2m)) since r ≥ 1
  have hsqrt_bound : Real.sqrt (Real.pi / (2 * (m * ‖z‖))) ≤ Real.sqrt (Real.pi / (2 * m)) := by
    apply Real.sqrt_le_sqrt
    apply div_le_div_of_nonneg_left (by linarith [Real.pi_pos]) (by positivity)
    nlinarith
  -- exp(-mr) ≤ 4/(mr)² = 4/(m²r²)
  have hexp_bound := exp_neg_le_four_div_sq hmr_pos
  calc 1 / (2 * Real.pi) * besselK0 (m * ‖z‖)
      ≤ 1 / (2 * Real.pi) * (Real.sqrt (Real.pi / (2 * m)) * (4 / (m * ‖z‖) ^ 2)) := by
        apply mul_le_mul_of_nonneg_left _ (by positivity)
        calc besselK0 (m * ‖z‖)
            ≤ Real.sqrt (Real.pi / (2 * (m * ‖z‖))) * Real.exp (-(m * ‖z‖)) := hK_bound
          _ ≤ Real.sqrt (Real.pi / (2 * m)) * (4 / (m * ‖z‖) ^ 2) :=
              mul_le_mul hsqrt_bound hexp_bound (by positivity) (Real.sqrt_nonneg _)
    _ = 2 * Real.sqrt (Real.pi / (2 * m)) / (Real.pi * m ^ 2) / ‖z‖ ^ (2:ℝ) := by
        simp only [show (2:ℝ) = ↑(2:ℕ) from by norm_num, Real.rpow_natCast]
        field_simp [ne_of_gt Real.pi_pos, ne_of_gt hm, ne_of_gt hz_pos]; ring

/-- **Exponential decay** of the free covariance kernel in d=2.
    |K(z)| ≤ (1/π)·e^{-m‖z‖} for ‖z‖ ≥ 1/m.
    Proved from besselK0_asymptotic: for ζ = m‖z‖ ≥ 1,
    √(π/(2ζ)) ≤ √(π/2) ≤ 2, so (1/(2π))·K₀(ζ) ≤ (1/(2π))·2·exp(-ζ) = (1/π)·exp(-ζ). -/
theorem freeCovarianceKernel_exponential_decay (m : ℝ) (hm : 0 < m) :
    ∀ z : SpaceTime, ‖z‖ ≥ 1/m →
      |freeCovarianceKernel m z| ≤ (1 / Real.pi) * Real.exp (-m * ‖z‖) := by
  intro z hz
  have hz_pos : 0 < ‖z‖ := by
    have : 0 < 1 / m := div_pos one_pos hm
    linarith
  have hz_ne : ‖z‖ ≠ 0 := ne_of_gt hz_pos
  have hmr_pos : 0 < m * ‖z‖ := mul_pos hm hz_pos
  have hmr_ge : 1 ≤ m * ‖z‖ := by
    have := mul_le_mul_of_nonneg_left hz hm.le
    simp [ne_of_gt hm] at this; linarith
  unfold freeCovarianceKernel freeCovariance freeCovarianceBessel
  simp only [zero_sub, norm_neg, hz_ne, ↓reduceIte]
  have hK_pos : 0 < besselK0 (m * ‖z‖) := besselK0_pos _ hmr_pos
  rw [abs_of_pos (mul_pos (by positivity : 0 < 1 / (2 * Real.pi)) hK_pos)]
  have hK_bound := besselK0_asymptotic (m * ‖z‖) hmr_pos
  -- √(π/(2ζ)) ≤ 2 for ζ ≥ 1
  have hsqrt_le : Real.sqrt (Real.pi / (2 * (m * ‖z‖))) ≤ 2 := by
    rw [Real.sqrt_le_left (by norm_num : (0:ℝ) ≤ 2)]
    have : Real.pi / (2 * (m * ‖z‖)) ≤ Real.pi / 2 :=
      div_le_div_of_nonneg_left (by linarith [Real.pi_pos]) (by positivity) (by linarith)
    nlinarith [Real.pi_le_four]
  calc 1 / (2 * Real.pi) * besselK0 (m * ‖z‖)
      ≤ 1 / (2 * Real.pi) * (2 * Real.exp (-(m * ‖z‖))) := by
        apply mul_le_mul_of_nonneg_left _ (by positivity)
        calc besselK0 (m * ‖z‖)
            ≤ Real.sqrt (Real.pi / (2 * (m * ‖z‖))) * Real.exp (-(m * ‖z‖)) := hK_bound
          _ ≤ 2 * Real.exp (-(m * ‖z‖)) :=
              mul_le_mul_of_nonneg_right hsqrt_le (Real.exp_nonneg _)
    _ = 1 / Real.pi * Real.exp (-m * ‖z‖) := by field_simp

/-- **Continuity of the free covariance kernel away from the origin.**

    The kernel C(z) = (1/(2π)) K₀(m‖z‖) is continuous on {z | z ≠ 0}.

    This follows from:
    - ‖z‖ is continuous
    - K₀ is continuous on (0, ∞) (see `besselK0_continuousOn`)
...
lemma freeCovarianceKernel_continuousOn (m : ℝ) (hm : 0 < m) :
  ...
  -- The kernel is f(‖z‖) where f(r) = (1/(2π)) K₀(mr)
  -- We show continuity by composition
  have hg_cont : ContinuousOn (fun r : ℝ => (1 / (2 * Real.pi)) * besselK0 (m * r))
      (Set.Ioi 0) := by
    apply ContinuousOn.mul continuousOn_const
    · apply besselK0_continuousOn.comp (continuousOn_const.mul continuousOn_id)
  ...
  have h_eq : ∀ z ∈ ({z : SpaceTime | z ≠ 0} : Set (SpaceTime)),
      freeCovarianceKernel m z = (1 / (2 * Real.pi)) * besselK0 (m * ‖z‖) := by
    intro z hz
    simp only [Set.mem_setOf_eq] at hz
    unfold freeCovarianceKernel freeCovariance freeCovarianceBessel
