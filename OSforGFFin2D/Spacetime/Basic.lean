...
-- Truly dimension-agnostic; imported unchanged from the 4D library.
import OSforGFFin2D.General.FunctionalAnalysis
import Minlos.NuclearSpace

/-!
# Basic Definitions (d = 2)

Core type definitions for the 2D formalization:

- `SpaceTime` = EuclideanSpace ℝ (Fin 2), the Euclidean 2-space ℝ²
...
/-- Spacetime dimension for the 2D port (Euclidean ℝ²). -/
abbrev STDimension := 2
...
def spatialPart (x : SpaceTime) : SpatialCoords :=
  (EuclideanSpace.equiv (Fin (STDimension - 1)) ℝ).symm
    (fun i => x ⟨i.val + 1, by have := i.isLt; simp [STDimension] at this ⊢⟩)
