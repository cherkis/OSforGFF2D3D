...
noncomputable def spacetimeOfTimeSpace (t : ℝ) (x : SpatialCoords1) : SpaceTime :=
  EuclideanSpace.equiv (Fin 2) ℝ |>.symm (Fin.cons t (fun i => x i))

/-- The time coordinate of spacetimeOfTimeSpace is t. -/
lemma spacetimeOfTimeSpace_time (t : ℝ) (x : SpatialCoords1) :
...
lemma spacetimeOfTimeSpace_spatial (t : ℝ) (x : SpatialCoords1) (i : Fin 1) :
    (spacetimeOfTimeSpace t x) ⟨i.val + 1, Nat.add_lt_add_right i.isLt 1⟩ = x i := by
  have h : (⟨i.val + 1, Nat.add_lt_add_right i.isLt 1⟩ : Fin 2) = Fin.succ i := rfl
  -- Fin 1 has only one element; i = 0
  have hi : i = 0 := Subsingleton.elim i 0
  simp [spacetimeOfTimeSpace, EuclideanSpace.equiv, h, hi]

/-- The decomposition: spacetimeOfTimeSpace t x = timeOrigin t + spatialEmbed x.
    This is the key structural fact: (t, x) = (t, 0) + (0, x). -/
lemma spacetimeOfTimeSpace_decompose (t : ℝ) (x : SpatialCoords1) :
...
lemma spacetimeOfTimeSpace_norm_ge (t : ℝ) (x : SpatialCoords1) :
  ...
  have hsq : ‖spacetimeOfTimeSpace t x‖ ^ 2 = t ^ 2 + ‖x‖ ^ 2 := by
    ...
    congr 1
    rw [EuclideanSpace.norm_sq_eq, Fin.sum_univ_one]
    simp only [Real.norm_eq_abs, sq_abs]
    -- At d=2, spatial = Fin 1 has only one component: x 0
...
noncomputable def spatialEmbedCLM : SpatialCoords1 →L[ℝ] SpaceTime :=
  ⟨spatialEmbed, spatialEmbed_continuous⟩

/-- The time-origin point: (t, 0, 0) in SpaceTime. -/
...
/-- A Schwartz function restricted to a fixed time slice is integrable over ℝ².
    Uses decay transfer: 3D Schwartz decay implies 2D integrability via norm comparison. -/
lemma schwartz_time_slice_integrable (f : TestFunctionℂ) (t : ℝ) :
    Integrable (fun x : SpatialCoords1 => f (spacetimeOfTimeSpace t x)) volume := by
  ...
  -- 2. spacetimeOfTimeSpace t x = (t, x₁, x₂), so ‖spacetimeOfTimeSpace t x‖² = t² + ‖x‖²
  -- 3. For fixed t, ‖spacetimeOfTimeSpace t x‖ ≥ ‖x‖
  -- 4. So |f(spacetimeOfTimeSpace t x)| ≤ C/(1 + ‖x‖)^N which is integrable on ℝ² for N > 2
  --
  -- Use Schwartz decay bound from FunctionalAnalysis
  have hST_dim : Module.finrank ℝ SpaceTime < 4 := by
    simp only [SpaceTime, finrank_euclideanSpace, Fintype.card_fin]
    norm_num
  obtain ⟨C, hC_pos, hf_decay⟩ := schwartz_integrable_decay f 4 hST_dim
  -- Note: SpatialCoords1 has dimension 2, and we need N > dim, so N = 4 > 2 works

  -- The dominator function: x ↦ C / (1 + ‖x‖)^4
  have h_dom_integrable : Integrable (fun x : SpatialCoords1 => C / (1 + ‖x‖)^4) volume := by
    have h_dim : (Module.finrank ℝ SpatialCoords1 : ℝ) < 4 := by
      simp only [finrank_euclideanSpace, Fintype.card_fin]
      norm_num
    have h_int := integrable_one_add_norm (E := SpatialCoords1) (μ := volume) (r := 4) h_dim
    -- Convert between (1+‖x‖)^(-4:ℝ) and C/(1+‖x‖)^4
    have h_eq : ∀ x : SpatialCoords1, C / (1 + ‖x‖) ^ 4 = C * (1 + ‖x‖) ^ (-(4 : ℝ)) := by
      intro x
      have h_pos : 0 < 1 + ‖x‖ := by linarith [norm_nonneg x]
      have h1 : ((1 + ‖x‖) ^ 4)⁻¹ = (1 + ‖x‖) ^ (-(4 : ℝ)) := by
        rw [← Real.rpow_natCast (1 + ‖x‖) 4, ← Real.rpow_neg (le_of_lt h_pos)]
        simp
      rw [div_eq_mul_inv, h1]
    simp_rw [h_eq]
    exact h_int.const_mul C

  -- Pointwise bound: |f(spacetimeOfTimeSpace t x)| ≤ C/(1+‖spacetimeOfTimeSpace t x‖)^4 ≤ C/(1+‖x‖)^4
  have h_bound : ∀ x : SpatialCoords1,
      ‖f (spacetimeOfTimeSpace t x)‖ ≤ C / (1 + ‖x‖)^4 := by
    intro x
...
lemma spatialNormIntegral_zero_of_neg (f : TestFunctionℂ)
  ...
  have h_zero : ∀ x : SpatialCoords1, ‖f (spacetimeOfTimeSpace t x)‖ = 0 := by
    intro x
...
lemma schwartz_vanishing_ftc_decay (f : TestFunctionℂ)
    (hf_supp : ∀ x : SpaceTime, x 0 ≤ 0 → f x = 0) :
    ∃ C : ℝ, 0 < C ∧ ∀ (t : ℝ) (_ht : 0 < t) (x_sp : SpatialCoords1),
  ...
  have h_dist : ‖x - x₀_bdy‖ = t := by
    ...
    have hsq : ‖spacetimeOfTimeSpace t 0‖^2 = t^2 := by
      rw [EuclideanSpace.norm_sq_eq, Fin.sum_univ_two]
      -- At d=2, only two components: time = t, spatial = 0
      ...
      simp only [h0, h1, Real.norm_eq_abs, abs_zero, sq_abs]
      ring
...
theorem spatialNormIntegral_linear_bound (f : TestFunctionℂ)
  ...
  have h_decay_int := polynomial_decay_integrable_1d

  -- Step 3: Define the constant K = ∫ 1/(1+‖x‖)^4 dx on ℝ² (finite by h_decay_int)
  let K := ∫ x : EuclideanSpace ℝ (Fin 1), 1 / (1 + ‖x‖)^4
  ...
  have h_pointwise : ∀ x : SpatialCoords1,
      ‖f (spacetimeOfTimeSpace t x)‖ ≤ C_pt * t / (1 + ‖x‖)^4 := fun x => h_pt_bound t ht x

  -- The bound function is integrable (scale of polynomial_decay_integrable_2d)
  have h_bound_int : Integrable (fun x : SpatialCoords1 => C_pt * t / (1 + ‖x‖)^4) volume := by
    have h1 : Integrable (fun x : SpatialCoords1 => 1 / (1 + ‖x‖)^4) volume := h_decay_int
    -- C_pt * t / (1 + ‖x‖)^4 = (C_pt * t) * (1 / (1 + ‖x‖)^4)
    have h_eq : (fun x : SpatialCoords1 => C_pt * t / (1 + ‖x‖)^4) =
                (fun x : SpatialCoords1 => (C_pt * t) * (1 / (1 + ‖x‖)^4)) := by
      ext x; ring
    rw [h_eq]
    exact h1.const_mul (C_pt * t)

  -- The integrand ‖f ...‖ is integrable (bounded by integrable function)
  have h_f_int : Integrable (fun x : SpatialCoords1 => ‖f (spacetimeOfTimeSpace t x)‖) volume := by
  ...
  have h_ae_bound : ∀ᵐ x ∂(volume : Measure SpatialCoords1),
  ...
  have h_factor : ∫ x : SpatialCoords1, C_pt * t / (1 + ‖x‖)^4 = C_pt * t * K := by
    have h_eq : (fun x : SpatialCoords1 => C_pt * t / (1 + ‖x‖)^4) =
                (fun x : SpatialCoords1 => (C_pt * t) * (1 / (1 + ‖x‖)^4)) := by ext x; ring
    rw [h_eq]
  ...
  calc spatialNormIntegral f t
      = ∫ x : SpatialCoords1, ‖f (spacetimeOfTimeSpace t x)‖ := rfl
    _ ≤ ∫ x : SpatialCoords1, C_pt * t / (1 + ‖x‖)^4 := h_mono
    _ = C_pt * t * K := h_factor
