...
lemma spacetime_norm_sq_decompose (k : SpaceTime) :
  ...
  -- At d=2, SpaceTime has 2 components (STDimension = 2)
  have hST : ‖k‖^2 = (k 0)^2 + (k 1)^2 := by
    rw [EuclideanSpace.norm_sq_eq, Fin.sum_univ_two]
    simp only [Real.norm_eq_abs, sq_abs]
  -- Spatial has 1 component (STDimension - 1 = 1)
  have hSp : ‖spatialPart k‖^2 = (k 1)^2 := by
    have h0 : (spatialPart k : Fin (STDimension - 1) → ℝ) ⟨0, by decide⟩ = k 1 := rfl
    simp only [EuclideanSpace.norm_sq_eq, Real.norm_eq_abs, sq_abs]
    -- Fin 1 sum: only one element
    have hUniv : (Finset.univ : Finset (Fin (STDimension - 1))) = {⟨0, by decide⟩} := rfl
    rw [hUniv, Finset.sum_singleton, h0]
  rw [hST, hSp]
