...
lemma real_inner_eq_mul (x y : ℝ) : @inner ℝ ℝ _ x y = x * y := by
  simp [real_inner_eq_re_inner (𝕜 := ℝ), RCLike.inner_apply, mul_comm]
...
/-- `spatialPart` is continuous as a coordinate projection followed by the standard equivalence. -/
lemma continuous_spatialPart : Continuous spatialPart := by
  ...
  apply continuous_pi
  intro i
  have h : i.val + 1 < STDimension := by simp [STDimension]
  ...
  have h_lintegral_finite : ∫⁻ p : ℝ × SpatialCoords, ENNReal.ofReal (G₀ p) ∂μ < ⊤ := by
    ...
    calc ∫⁻ s in Set.Ioi (0 : ℝ), ∫⁻ k : SpatialCoords,
      ...
      _ < ⊤ := by
          ...
          -- At d=2, SpatialCoords = EuclideanSpace ℝ (Fin 1), finrank = 1
          -- Gaussian integral: ∫ exp(-s|k|²) dk = (π/s)^(1/2) = √(π/s)
          -- So s^(3/2) * exp(-sm²) * √(π/s) = √π * s * exp(-sm²)
          have h_inner : ∀ s ∈ Set.Ioi (0 : ℝ),
              ∫⁻ k : SpatialCoords, ENNReal.ofReal (s ^ (3/2 : ℝ) * Real.exp (-s * m^2) *
                Real.exp (-s * ‖k‖^2)) = ENNReal.ofReal (Real.sqrt π * s * Real.exp (-s * m^2)) := by
            intro s hs
            have hs_pos : 0 < s := hs
            ...
            -- Use Gaussian formula: ∫ exp(-s|k|²) dk = (π/s)^(finrank/2 : ℝ)
            -- At d=2, finrank(SpatialCoords) = 1, so integral = (π/s)^(1/2) = √(π/s)
            have h_dim : Module.finrank ℝ SpatialCoords = 1 := by
            ...
            -- h_gauss_val : ∫ v, rexp (-s * ‖v‖^2) = (π/s)^(1/2 : ℝ)
            have h_gauss_val' : ∫ a : SpatialCoords, Real.exp (-s * ‖a‖^2) = Real.sqrt (π / s) := by
              rw [Real.sqrt_eq_rpow]
              convert h_gauss_val using 2
              ring_nf
            rw [h_gauss_val']
            -- Now: s^(3/2) * exp(-sm²) * √(π/s) = √π * s * exp(-sm²)
            congr 1
            have hs_ne : s ≠ 0 := ne_of_gt hs_pos
            have h_sqrt_split : Real.sqrt (π / s) = Real.sqrt π / Real.sqrt s :=
              Real.sqrt_div' π (le_of_lt hs_pos)
            rw [h_sqrt_split, Real.sqrt_eq_rpow s]
            have h_s32_div : s ^ (3/2 : ℝ) / s ^ (1/2 : ℝ) = s := by
              rw [← Real.rpow_sub hs_pos]
              norm_num
            calc s ^ (3/2 : ℝ) * Real.exp (-s * m^2) * (Real.sqrt π / s ^ (1/2 : ℝ))
                = Real.sqrt π * (s ^ (3/2 : ℝ) / s ^ (1/2 : ℝ)) * Real.exp (-s * m^2) := by ring
              _ = Real.sqrt π * s * Real.exp (-s * m^2) := by rw [h_s32_div]
          ...
          have h_eqon : Set.EqOn
              ...
              (fun s => ENNReal.ofReal (Real.sqrt π * s * Real.exp (-s * m^2)))
              (Set.Ioi 0) := by
            intro s hs
            exact h_inner s hs
          have h_lintegral_eq : ∫⁻ s in Set.Ioi (0 : ℝ), ∫⁻ k : SpatialCoords,
              ...
              ∫⁻ s in Set.Ioi (0 : ℝ),
                ENNReal.ofReal (Real.sqrt π * s * Real.exp (-s * m^2)) ∂volume := by
            exact MeasureTheory.setLIntegral_congr_fun measurableSet_Ioi h_eqon
          calc ∫⁻ s in Set.Ioi (0 : ℝ), ∫⁻ k : SpatialCoords,
                ENNReal.ofReal (s ^ (3/2 : ℝ) * Real.exp (-s * m^2) *
                  Real.exp (-s * ‖k‖^2)) ∂volume ∂volume
            = ∫⁻ s in Set.Ioi (0 : ℝ),
                ENNReal.ofReal (Real.sqrt π * s * Real.exp (-s * m^2)) ∂volume := h_lintegral_eq
            _ < ⊤ := by
              -- At d=2: ∫_0^∞ √π * s * exp(-sm²) ds < ∞ (Laplace transform of s at m²)
              have h_exp_int : IntegrableOn (fun s => s ^ (1 : ℝ) * Real.exp (-m^2 * s)) (Set.Ioi 0) := by
                have hm2 : 0 < m ^ 2 := sq_pos_of_pos hm
                have hr : (-1 : ℝ) < (1 : ℝ) := by norm_num
              ...
              have h_prod_int : IntegrableOn (fun s => Real.sqrt π * s * Real.exp (-s * m^2)) (Set.Ioi 0) := by
                have h2 : IntegrableOn (fun s => Real.sqrt π * (s ^ (1 : ℝ) * Real.exp (-m^2 * s))) (Set.Ioi 0) :=
                  h_exp_int.const_mul (Real.sqrt π)
...
lemma spacetime_fubini_linear_vanishing_bound (f : TestFunctionℂ)
  ...
  have hG_meas : Measurable G := by
    ...
    have h_sts_cont : Continuous (Function.uncurry spacetimeOfTimeSpace) := by
      ...
      -- Need: Continuous (fun p : ℝ × SpatialCoords1 => Fin.cons p.1 (fun i => p.2 i))
      apply continuous_pi
      intro j
    ...
    have h_joint_cont : Continuous (fun p : ℝ × SpatialCoords1 =>
    ...
    have h_joint_sm : MeasureTheory.StronglyMeasurable (fun p : ℝ × SpatialCoords1 =>
  ...
  calc ∫ x : SpaceTime, ∫ y : SpaceTime,
    ...
    _ = ∫ t₁ : ℝ, ∫ t₂ : ℝ, K t₁ t₂ * G t₁ * G t₂ := by
        dsimp only at h_tonelli
        refine h_tonelli.trans ?_
        congr 1; ext t₁; congr 1; ext t₂
        rw [← congr_fun hG_eq t₁, ← congr_fun hG_eq t₂]; rfl
...
/-- For fixed `s`, the `(x,y)` integrand used in `fubini_s_xy_swap` is integrable. -/
lemma fubini_s_xy_fixed_integrand_integrable (m : ℝ) [Fact (0 < m)]
    (f : TestFunctionℂ) (k_sp : SpatialCoords) (s : ℝ) :
    Integrable
      (fun xy : SpaceTime × SpaceTime =>
        (starRingEnd ℂ (f xy.1)) * f xy.2 *
          (Real.sqrt (π / s) : ℂ) * Complex.exp (-((-(xy.1 0) - xy.2 0)^2 / (4 * s) : ℝ)) *
          Complex.exp (-(s : ℂ) * (‖k_sp‖^2 + m^2)) *
          Complex.exp (-Complex.I * spatialDot k_sp (spatialPart xy.1 - spatialPart xy.2)))
      (volume.prod volume) := by
  by_cases hs : 0 < s
      ...
      have h6 : Continuous (fun xy : SpaceTime × SpaceTime =>
        ...
        have hv_i : Continuous (fun xy : SpaceTime × SpaceTime =>
          ...
          (PiLp.continuous_apply 2 (fun _ : Fin (STDimension - 1) => ℝ) i).comp
            ((continuous_spatialPart.comp continuous_fst).sub
...
theorem fubini_s_xy_swap (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ) (k_sp : SpatialCoords) :
  ...
  have h_int : Integrable F ((volume.restrict (Set.Ioi 0)).prod (volume.prod volume)) := by
    ...
    · have h_ae : ∀ᵐ p : ℝ × SpaceTime × SpaceTime ∂(volume.restrict (Set.Ioi 0)).prod (volume.prod volume),
        ...
        have h_compl : ({p : ℝ × SpaceTime × SpaceTime | 0 < p.1})ᶜ = Prod.fst ⁻¹' Set.Iic 0 := by
          ext p
          simp only [Set.mem_compl_iff, Set.mem_setOf_eq, not_lt, Set.mem_preimage, Set.mem_Iic]
        rw [h_compl]
  ...
  calc ∫ s in Set.Ioi 0, ∫ x : SpaceTime, ∫ y : SpaceTime, F (s, x, y)
      = ∫ s in Set.Ioi 0, ∫ xy : SpaceTime × SpaceTime, F (s, xy.1, xy.2) := by
          congr 1 with s
          symm
          have h_int_s := fubini_s_xy_fixed_integrand_integrable m f k_sp s
