...
/-- The GFF two-point Schwinger function satisfies a logarithmic decay bound.
    In d=2, the free covariance kernel is (1/(2π)) K₀(m‖z‖), which behaves like
    -log(m‖z‖/2) near the origin.
    - Near origin: K₀(mr) ~ -log(mr/2), giving logarithmic singularity
    - Far from origin: K₀(mr) ~ √(π/(2mr)) exp(-mr), exponential decay -/
theorem schwinger_two_point_decay_bound_GFF (m : ℝ) [Fact (0 < m)] :
  ∃ C : ℝ, C > 0 ∧
    ∀ x y : SpaceTime,
      ‖SchwingerTwoPointFunction_GFF m (x - y)‖ ≤
        C * (|Real.log (m * ‖x - y‖)| + 1) := by
...
/-- The abstract two-point Schwinger function satisfies a logarithmic decay bound.
    Uses the bridge lemma to connect to the concrete GFF definition.
    In d=2, the singularity of the free covariance is logarithmic (not power-law). -/
...
/-- The two-point Schwinger function is locally integrable.
    In d=2, the free covariance kernel C(x) = (1/(2π)) K₀(m‖x‖) is globally integrable
    (since K₀ has only a logarithmic singularity at the origin and exponential decay at infinity),
    hence locally integrable. The abstract SchwingerTwoPointFunction agrees with the kernel a.e. -/
lemma gff_two_point_locally_integrable (m : ℝ) [Fact (0 < m)] :
  ...
  -- freeCovarianceKernel is globally integrable at d=2 (logarithmic singularity)
  have h_kernel_int : Integrable (freeCovarianceKernel m) volume :=
    freeCovarianceKernel_integrable m (Fact.out)
  -- SchwingerTwoPointFunction equals freeCovarianceKernel a.e.
  have h_ae_eq : (fun x => SchwingerTwoPointFunction (gaussianFreeField_free m) x) =ᶠ[ae volume]
                 freeCovarianceKernel m := by
    have h_singleton_null : (volume : Measure SpaceTime) {(0 : SpaceTime)} = 0 :=
      MeasureTheory.measure_singleton (0 : SpaceTime)
    ...
    exact Filter.eventuallyEq_of_mem h_mem
      (fun x hx => schwingerTwoPointFunction_eq_freeCovarianceKernel m x hx)
  -- Global integrability of the kernel implies local integrability
  -- and agreeing a.e. transfers local integrability
  exact (h_kernel_int.locallyIntegrable).congr h_ae_eq.symm
