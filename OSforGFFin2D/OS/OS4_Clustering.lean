...
import OSforGFFin2D.Measure.GaussianFreeField -- For gaussian_satisfies_OS2
import OSforGFFin2D.OS.OS2_Invariance -- For CovarianceEuclideanInvariantℂ_μ_GFF
import OSforGFFin2D.Covariance.Position  -- For freeCovarianceℂ_bilinear_symm
...
import OSforGFFin2D.Spacetime.TimeTranslation  -- For time translation on distributions
import OSforGFFin2D.OS.OS4_MGF  -- For shared OS4 infrastructure (no sorries)
...
theorem schwartz_cross_covariance_decay_real (m : ℝ) [Fact (0 < m)]
  ...
  -- Step 1: Get the polynomial decay bound for the kernel
  have hm : 0 < m := Fact.out
  obtain ⟨C, hC_pos, hK_decay'⟩ := freeCovarianceKernel_polynomial_bound m hm
...
/-- The time shift constant vector (s, 0) has norm |s|. -/
...
lemma freeCovariance_eq_kernel (m : ℝ) (x y : SpaceTime) :
  ...
  -- freeCovariance m x y = freeCovarianceBessel m x y = (1/(2π)) K₀(mr) where r = ‖x - y‖
  -- freeCovarianceKernel m z = freeCovariance m 0 z = freeCovarianceBessel m 0 z
  --   = (1/(2π)) K₀(mr') where r' = ‖0 - z‖ = ‖z‖
  -- For z = x - y: r' = ‖x - y‖ = r
  unfold freeCovarianceKernel freeCovariance freeCovarianceBessel
  -- Goal: ‖x - y‖ = ‖0 - (x - y)‖
...
theorem gaussianFreeField_satisfies_OS4_PolynomialClustering (m : ℝ) [Fact (0 < m)]
  ...
  -- For d=2: the constant involves K₀ asymptotics: (1/(2π)) * 2 * √(π/(2*mr)) * e^{-mr}
  -- Simplified upper bound: C_exp ≤ 1/π for mr ≥ 1 (since √(π/(2*mr)) ≤ √(π/2))
  let C_exp := (1 / Real.pi : ℝ)
