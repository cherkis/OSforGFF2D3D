...
/-- The measurable equivalence from SpaceTime to ℝ × SpatialCoords.
    ...
    1. piLpMeasurableEquiv : EuclideanSpace ℝ (Fin 3) → (Fin 3 → ℝ)
    2. piFinSuccAbove 0 : (Fin 3 → ℝ) → ℝ × (Fin 2 → ℝ)
    3. id × piLpMeasurableEquiv.symm : ℝ × (Fin 2 → ℝ) → ℝ × SpatialCoords -/
...
theorem spacetimeDecomp_measurePreserving :
  ...
  -- Step 1: PiLp → (Fin 3 → ℝ) is measure-preserving
...
lemma spacetime_norm_sq_decompose (k : SpaceTime) :
  ...
  -- Expand SpaceTime norm as sum over 3 components (STDimension = 3)
  have hST : ‖k‖^2 = (k 0)^2 + (k 1)^2 + (k 2)^2 := by
    rw [EuclideanSpace.norm_sq_eq, Fin.sum_univ_three]
    simp only [Real.norm_eq_abs, sq_abs]
  -- Expand SpatialCoords norm as sum over 2 components (STDimension - 1 = 2)
  have hSp : ‖spatialPart k‖^2 = (k 1)^2 + (k 2)^2 := by
