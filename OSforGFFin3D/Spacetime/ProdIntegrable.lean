...
abbrev SpatialCoords2 : Type := EuclideanSpace ℝ (Fin 2)

/-- Decomposition of SpaceTime as time × space. -/
noncomputable def spacetimeOfTimeSpace (t : ℝ) (x : SpatialCoords2) : SpaceTime :=
  EuclideanSpace.equiv (Fin 3) ℝ |>.symm (Fin.cons t (fun i => x i))

/-- The time coordinate of spacetimeOfTimeSpace is t. -/
lemma spacetimeOfTimeSpace_time (t : ℝ) (x : SpatialCoords2) :
...
lemma spacetimeOfTimeSpace_spatial (t : ℝ) (x : SpatialCoords2) (i : Fin 2) :
    (spacetimeOfTimeSpace t x) ⟨i.val + 1, Nat.add_lt_add_right i.isLt 1⟩ = x i := by
  have h : (⟨i.val + 1, Nat.add_lt_add_right i.isLt 1⟩ : Fin 3) = Fin.succ i := rfl
...
lemma spacetimeOfTimeSpace_decompose (t : ℝ) (x : SpatialCoords2) :
...
lemma spacetimeOfTimeSpace_norm_ge (t : ℝ) (x : SpatialCoords2) :
...
noncomputable def spatialEmbed : SpatialCoords2 →ₗ[ℝ] SpaceTime where
...
noncomputable def spatialEmbedCLM : SpatialCoords2 →L[ℝ] SpaceTime :=
...
lemma schwartz_time_slice_integrable (f : TestFunctionℂ) (t : ℝ) :
    Integrable (fun x : SpatialCoords2 => f (spacetimeOfTimeSpace t x)) volume := by
  ...
  -- Note: SpatialCoords2 has dimension 2, and we need N > dim, so N = 4 > 2 works

  -- The dominator function: x ↦ C / (1 + ‖x‖)^4
  have h_dom_integrable : Integrable (fun x : SpatialCoords2 => C / (1 + ‖x‖)^4) volume := by
    have h_dim : (Module.finrank ℝ SpatialCoords2 : ℝ) < 4 := by
      simp only [finrank_euclideanSpace, Fintype.card_fin]
      norm_num
    have h_int := integrable_one_add_norm (E := SpatialCoords2) (μ := volume) (r := 4) h_dim
    -- Convert between (1+‖x‖)^(-4:ℝ) and C/(1+‖x‖)^4
    have h_eq : ∀ x : SpatialCoords2, C / (1 + ‖x‖) ^ 4 = C * (1 + ‖x‖) ^ (-(4 : ℝ)) := by
      intro x
...
noncomputable def spatialNormIntegral (f : TestFunctionℂ) (t : ℝ) : ℝ :=
  ∫ x : SpatialCoords2, ‖f (spacetimeOfTimeSpace t x)‖
...
lemma spatialNormIntegral_zero_of_neg (f : TestFunctionℂ)
  ...
  have h_zero : ∀ x : SpatialCoords2, ‖f (spacetimeOfTimeSpace t x)‖ = 0 := by
    intro x
...
lemma schwartz_vanishing_ftc_decay (f : TestFunctionℂ)
    (hf_supp : ∀ x : SpaceTime, x 0 ≤ 0 → f x = 0) :
    ∃ C : ℝ, 0 < C ∧ ∀ (t : ℝ) (_ht : 0 < t) (x_sp : SpatialCoords2),
  ...
  have h_dist : ‖x - x₀_bdy‖ = t := by
    ...
    have hsq : ‖spacetimeOfTimeSpace t 0‖^2 = t^2 := by
      ...
      simp only [h0, h1, h2, Real.norm_eq_abs, abs_zero, sq_abs]
      ring
...
theorem spatialNormIntegral_linear_bound (f : TestFunctionℂ)
  ...
  have h_decay_int := polynomial_decay_integrable_2d

  -- Step 3: Define the constant K = ∫ 1/(1+‖x‖)^4 dx on ℝ² (finite by h_decay_int)
  let K := ∫ x : EuclideanSpace ℝ (Fin 2), 1 / (1 + ‖x‖)^4
  ...
  have h_pointwise : ∀ x : SpatialCoords2,
  ...
  have h_bound_int : Integrable (fun x : SpatialCoords2 => C_pt * t / (1 + ‖x‖)^4) volume := by
    have h1 : Integrable (fun x : SpatialCoords2 => 1 / (1 + ‖x‖)^4) volume := h_decay_int
    -- C_pt * t / (1 + ‖x‖)^4 = (C_pt * t) * (1 / (1 + ‖x‖)^4)
    have h_eq : (fun x : SpatialCoords2 => C_pt * t / (1 + ‖x‖)^4) =
                (fun x : SpatialCoords2 => (C_pt * t) * (1 / (1 + ‖x‖)^4)) := by
      ext x; ring
    rw [h_eq]
    exact h1.const_mul (C_pt * t)

  -- The integrand ‖f ...‖ is integrable (bounded by integrable function)
  have h_f_int : Integrable (fun x : SpatialCoords2 => ‖f (spacetimeOfTimeSpace t x)‖) volume := by
  ...
  have h_ae_bound : ∀ᵐ x ∂(volume : Measure SpatialCoords2),
  ...
  have h_factor : ∫ x : SpatialCoords2, C_pt * t / (1 + ‖x‖)^4 = C_pt * t * K := by
    have h_eq : (fun x : SpatialCoords2 => C_pt * t / (1 + ‖x‖)^4) =
                (fun x : SpatialCoords2 => (C_pt * t) * (1 / (1 + ‖x‖)^4)) := by ext x; ring
    rw [h_eq]
  ...
  calc spatialNormIntegral f t
      = ∫ x : SpatialCoords2, ‖f (spacetimeOfTimeSpace t x)‖ := rfl
    _ ≤ ∫ x : SpatialCoords2, C_pt * t / (1 + ‖x‖)^4 := h_mono
    _ = C_pt * t * K := h_factor
