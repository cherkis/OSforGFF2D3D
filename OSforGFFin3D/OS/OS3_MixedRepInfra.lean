...
/-- **THEOREM** (formerly axiom): The d-dimensional Gaussian Fourier transform.
    ...
    For d = 3, this states:
    (1/(2π)^3) ∫_{ℝ^3} exp(-ik·z) exp(-s|k|²) d³k = (4πs)^{-3/2} exp(-|z|²/(4s))
...
lemma spacetime_fubini_linear_vanishing_bound (f : TestFunctionℂ)
  ...
  have hG_meas : Measurable G := by
    ...
    have h_sts_cont : Continuous (Function.uncurry spacetimeOfTimeSpace) := by
      ...
      -- Need: Continuous (fun p : ℝ × SpatialCoords2 => Fin.cons p.1 (fun i => p.2 i))
      apply continuous_pi
      intro j
    ...
    have h_joint_cont : Continuous (fun p : ℝ × SpatialCoords2 =>
    ...
    have h_joint_sm : MeasureTheory.StronglyMeasurable (fun p : ℝ × SpatialCoords2 =>
