...
import OSforGFFin3D.Measure.GaussianFreeField -- For gaussian_satisfies_OS2
import OSforGFFin3D.OS.OS2_Invariance -- For CovarianceEuclideanInvariantℂ_μ_GFF
import OSforGFFin3D.Covariance.Position  -- For freeCovarianceℂ_bilinear_symm
...
import OSforGFFin3D.Spacetime.TimeTranslation  -- For time translation on distributions
import OSforGFFin3D.OS.OS4_MGF  -- For shared OS4 infrastructure (no sorries)
...
theorem schwartz_cross_covariance_decay_real (m : ℝ) [Fact (0 < m)]
  ...
  -- Step 2: Convert the decay bound to the form needed by the decay theorem
  -- The theorem wants: |K(z)| ≤ C / ‖z‖^α for ‖z‖ ≥ R₀
