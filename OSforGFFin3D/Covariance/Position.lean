...
Position-space covariance C(x,y) = (1/(4π)^{3/2})·2·(2m/|x−y|)^{1/2}·K_{1/2}(m|x−y|) and its properties:
...
/-- ** (Time Reflection Invariance - Position Space):**
  ...
  This follows from general Euclidean invariance since time reflection is in O(3). -/
...
/-- **(Parseval for Bessel Covariance):**
    ...
    Note: Unlike the regulated form, this uses freeCovariance (Bessel K_{1/2} form) which is
