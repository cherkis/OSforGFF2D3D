...
In the 3D spacetime branch (d = 3), this simplifies to:

  C(r) = (1 / (4π)^(3/2)) ∫₀^∞ e^{-tm²} · t^(-3/2) · e^{-r²/(4t)} dt

The remaining 1D integral can be computed via the substitution u = m²t + r²/(4t),
which leads to the half-order Bessel function `besselKhalf = besselK (1/2)`.
-/
...
lemma heatKernelPositionSpace_3D (t : ℝ) (ht : 0 < t) (r : ℝ) :
  ...
  rw [show (-(STDimension : ℝ) / 2) = -(3 / 2 : ℝ) by simp [STDimension]; norm_num]
  rw [Real.rpow_neg (le_of_lt hpos), one_div]
...
lemma heatKernelPositionSpace_bounded (r : ℝ) (hr : 0 < r) :
  ...
  let const : ℝ := 1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))
  let g : ℝ → ℝ := fun u => u ^ (3 / 2 : ℝ) * Real.exp (-(r^2 / 4) * u)
  have hconst_pos : 0 < const := by
    unfold const
    positivity
  have htail : ∀ᶠ u : ℝ in Filter.atTop, g u < 1 := by
    have h_tendsto := tendsto_rpow_mul_exp_neg_mul_atTop_nhds_zero (3 / 2 : ℝ) (r^2 / 4) (by positivity)
    have h_nhds : Set.Iio (1 : ℝ) ∈ nhds (0 : ℝ) := Iio_mem_nhds zero_lt_one
  ...
  have hg_cont : ContinuousOn g (Set.Icc 1 A) := by
    unfold g
    ...
    · have hcont : ContinuousOn (fun u : ℝ => -(r^2 / 4) * u) (Set.Icc 1 A) := by
        fun_prop
      exact Real.continuous_exp.continuousOn.comp hcont (by intro u _; exact Set.mem_univ _)
  rcases IsCompact.exists_bound_of_continuousOn isCompact_Icc hg_cont with ⟨M0, hM0⟩
  let M : ℝ := max M0 1
  have hM_pos : 0 < M := by
    unfold M
    linarith [le_max_right M0 1]
  have hg_nonneg : ∀ u : ℝ, 1 ≤ u → 0 ≤ g u := by
    intro u hu
    unfold g
    apply mul_nonneg
    · exact Real.rpow_nonneg (le_trans zero_le_one hu) (3 / 2 : ℝ)
    · exact Real.exp_nonneg _
  have hg_bound : ∀ u : ℝ, 1 ≤ u → g u ≤ M := by
    intro u hu
    by_cases huA : u ≤ A
    · have hM0u : ‖g u‖ ≤ M0 := hM0 u ⟨hu, huA⟩
      have hgu : g u ≤ M0 := by
        rw [Real.norm_of_nonneg (hg_nonneg u hu)] at hM0u
        exact hM0u
      exact hgu.trans (le_max_left _ _)
    · have huN : N ≤ u := by
        unfold A at huA
        have hAu : A < u := lt_of_not_ge huA
        exact le_trans (le_max_left N 1) (le_of_lt hAu)
      have htail_u : g u < 1 := hN u huN
  ...
  have h_repr : ∀ s > 0, heatKernelPositionSpace s r = const * g (1 / s) := by
    ...
    calc
      ...
      _ = const * g (1 / s) := by
        unfold g
        rfl
  let C : ℝ := max (const * M) const
  refine ⟨C, ?_, ?_⟩
  · unfold C
    positivity
  · intro s hs
    by_cases hs1 : s ≤ 1
    · have hs_inv_ge_one : 1 ≤ 1 / s := by
        field_simp [hs.ne']
        linarith
      rw [h_repr s hs]
      have hsmall : g (1 / s) ≤ M := hg_bound (1 / s) hs_inv_ge_one
      have hbound : const * g (1 / s) ≤ const * M := by
    ...
    · have hs_gt : 1 < s := lt_of_not_ge hs1
      ...
      have h_exp_le : Real.exp (-r^2 / (4 * s)) ≤ 1 := by
        ...
        have hneg : -r^2 / (4 * s) ≤ 0 := by
          exact div_nonpos_of_nonpos_of_nonneg hnum_nonpos (le_of_lt hden_pos)
        exact Real.exp_le_one_iff.mpr hneg
      have hs_pow_ge_one : 1 ≤ s ^ (3 / 2 : ℝ) := by
        exact Real.one_le_rpow (by linarith : 1 ≤ s) (by positivity : 0 ≤ (3 / 2 : ℝ))
      have hs_invpow_le_one : 1 / s ^ (3 / 2 : ℝ) ≤ 1 := by
        simpa using (one_div_le_one_div_of_le zero_lt_one hs_pow_ge_one)
      calc
        1 / ((4 * Real.pi * s) ^ (3 / 2 : ℝ)) * Real.exp (-r^2 / (4 * s))
...
theorem heatKernelPositionSpace_integral_eq_one (t : ℝ) (ht : 0 < t) :
  ...
  rw [show ((3 : ℝ) / 2) = (3 / 2 : ℝ) by norm_num]
  rw [show (-(3 : ℝ) / 2) = -(3 / 2 : ℝ) by norm_num]
  rw [Real.rpow_neg (le_of_lt h_pos)]
  have h_ne : (4 * π * t) ^ (3 / 2 : ℝ) ≠ 0 := by
    apply ne_of_gt
...
theorem covarianceSchwingerRep_eq_besselFormula (m r : ℝ) (hm : 0 < m) (hr : 0 < r) :
  ...
  have h_eq : ∫ t in Set.Ioi 0, Real.exp (-t * m^2) * (1 / t ^ (3 / 2 : ℝ)) * Real.exp (-r^2 / (4 * t)) =
    ...
    intro t _ht
    have h1 : Real.exp (-t * m^2) * (1 / t ^ (3 / 2 : ℝ)) * Real.exp (-r^2 / (4 * t)) =
        (1 / t ^ (3 / 2 : ℝ)) * (Real.exp (-t * m^2) * Real.exp (-r^2 / (4 * t))) := by ring
    calc Real.exp (-t * m^2) * (1 / t ^ (3 / 2 : ℝ)) * Real.exp (-r^2 / (4 * t))
      = (1 / t ^ (3 / 2 : ℝ)) * (Real.exp (-t * m^2) * Real.exp (-r^2 / (4 * t))) := h1
      _ = (1 / t ^ (3 / 2 : ℝ)) * Real.exp (-m^2 * t - r^2 / (4 * t)) := by
        rw [← Real.exp_add]
        congr 1
        ring_nf
  rw [h_eq, schwingerIntegral_eq_besselKhalf m r hm hr]

/-- The free covariance in position space via Bessel function representation.
    In the 3D spacetime branch this is the `besselKhalf` formula.

    This is the explicit formula for the massive scalar field propagator in 3D.
...
/-- Integrability of `exp (-t m^2)` on `(0, ∞)` for `m > 0`. -/
...
/-- Integrability of `exp (-t m^2) * C` on `(0, ∞)` for any constant `C`. -/
...
/-- The Gaussian Fourier transform gives the heat kernel, up to the `(2π)^d` normalization. -/
lemma gaussianFT_eq_heatKernel_times_norm (s : ℝ) (hs : 0 < s) (z : SpaceTime) :
  ...
  rw [h_exp_three_half]
  have h_pos_div : 0 < Real.pi / s := div_pos Real.pi_pos hs
  have h_lhs :
      (↑Real.pi / ↑s : ℂ) ^ (↑(3 / 2 : ℝ) : ℂ) * ↑(Real.exp (-‖z‖ ^ 2 / (4 * s))) =
      (↑((Real.pi / s) ^ (3 / 2 : ℝ) * Real.exp (-‖z‖ ^ 2 / (4 * s))) : ℂ) := by
    have h_div : (↑Real.pi / ↑s : ℂ) = (↑(Real.pi / s : ℝ) : ℂ) := by
      push_cast
      ring
    rw [h_div, (Complex.ofReal_cpow (le_of_lt h_pos_div) (3 / 2)).symm]
    simp only [Complex.ofReal_mul]
  set a : ℝ := (2 * Real.pi) ^ STDimension with ha_def
  set b : ℝ := (1 / ((4 * Real.pi * s) ^ (3 / 2 : ℝ))) * Real.exp (-‖z‖ ^ 2 / (4 * s)) with hb_def
  have h_rhs : (↑a : ℂ) * ↑b = (↑(a * b) : ℂ) := (Complex.ofReal_mul a b).symm
  have h_four : (4 : ℝ) ^ (3 / 2 : ℝ) = 8 := by
    have h_half : (4 : ℝ) ^ (1 / 2 : ℝ) = 2 := by
      rw [show (4 : ℝ) ^ (1 / 2 : ℝ) = Real.sqrt 4 by rw [← Real.sqrt_eq_rpow]]
      norm_num
    calc
      (4 : ℝ) ^ (3 / 2 : ℝ) = (4 : ℝ) ^ ((1 / 2 : ℝ) * 3) := by congr 2; norm_num
      _ = ((4 : ℝ) ^ (1 / 2 : ℝ)) ^ (3 : ℝ) := by
            rw [Real.rpow_mul (by positivity : 0 ≤ (4 : ℝ)) (1 / 2 : ℝ) (3 : ℝ)]
      _ = (2 : ℝ) ^ (3 : ℝ) := by rw [h_half]
      _ = 8 := by norm_num
  have h_real : (Real.pi / s) ^ (3 / 2 : ℝ) * Real.exp (-‖z‖ ^ 2 / (4 * s)) = a * b := by
    simp only [ha_def, hb_def]
    have h_div_rpow : (Real.pi / s) ^ (3 / 2 : ℝ) =
        Real.pi ^ (3 / 2 : ℝ) / s ^ (3 / 2 : ℝ) := by
      rw [Real.div_rpow (le_of_lt Real.pi_pos) (le_of_lt hs)]
    rw [h_div_rpow]
    have h_den : (4 * Real.pi * s) ^ (3 / 2 : ℝ) =
        8 * Real.pi ^ (3 / 2 : ℝ) * s ^ (3 / 2 : ℝ) := by
      calc
        (4 * Real.pi * s) ^ (3 / 2 : ℝ)
            = ((4 * Real.pi) * s) ^ (3 / 2 : ℝ) := by ring
        _ = (4 * Real.pi) ^ (3 / 2 : ℝ) * s ^ (3 / 2 : ℝ) := by
              rw [Real.mul_rpow (by positivity : 0 ≤ 4 * Real.pi) (le_of_lt hs)]
        _ = ((4 : ℝ) * Real.pi) ^ (3 / 2 : ℝ) * s ^ (3 / 2 : ℝ) := by ring
        _ = ((4 : ℝ) ^ (3 / 2 : ℝ) * Real.pi ^ (3 / 2 : ℝ)) * s ^ (3 / 2 : ℝ) := by
              rw [Real.mul_rpow (by positivity : 0 ≤ (4 : ℝ)) (le_of_lt Real.pi_pos)]
          _ = 8 * Real.pi ^ (3 / 2 : ℝ) * s ^ (3 / 2 : ℝ) := by rw [h_four]
    rw [h_den]
    have h_pi3 : (Real.pi ^ (3 : ℕ) : ℝ) = Real.pi ^ (3 : ℝ) := (Real.rpow_natCast Real.pi 3).symm
    rw [show ((2 * Real.pi) ^ (3 : ℕ) : ℝ) = 8 * Real.pi ^ (3 : ℕ) by ring, h_pi3]
    have h_pi_split : Real.pi ^ (3 : ℝ) = Real.pi ^ (3 / 2 : ℝ) * Real.pi ^ (3 / 2 : ℝ) := by
      calc
        Real.pi ^ (3 : ℝ) = Real.pi ^ ((3 / 2 : ℝ) * 2) := by congr 2; norm_num
        _ = (Real.pi ^ (3 / 2 : ℝ)) ^ (2 : ℝ) := by
          rw [Real.rpow_mul (le_of_lt Real.pi_pos) (3 / 2 : ℝ) (2 : ℝ)]
        _ = Real.pi ^ (3 / 2 : ℝ) * Real.pi ^ (3 / 2 : ℝ) := by simp [pow_two]
    rw [h_pi_split]
    have h_pi_ne : Real.pi ^ (3 / 2 : ℝ) ≠ 0 := by
      exact ne_of_gt (Real.rpow_pos_of_pos Real.pi_pos _)
    have h_s_ne : s ^ (3 / 2 : ℝ) ≠ 0 := by
      exact ne_of_gt (Real.rpow_pos_of_pos hs _)
    field_simp [h_pi_ne, h_s_ne]
  calc
    (↑Real.pi / ↑s : ℂ) ^ (↑(3 / 2 : ℝ) : ℂ) * ↑(Real.exp (-‖z‖ ^ 2 / (4 * s)))
        = ↑((Real.pi / s) ^ (3 / 2 : ℝ) * Real.exp (-‖z‖ ^ 2 / (4 * s))) := h_lhs
    _ = ↑(a * b) := by rw [h_real]
    _ = (↑a : ℂ) * ↑b := h_rhs.symm

/-- Integrability of the absolute-value Schwinger/Fourier integrand on `SpaceTime × ℝ`. -/
lemma integrable_schwinger_fourier_integrand (α : ℝ) (hα : 0 < α) (m : ℝ) (hm : 0 < m) :
    Integrable
      (fun p : SpaceTime × ℝ =>
        if p.2 > 0 then Real.exp (-(α + p.2) * ‖p.1‖^2 - p.2 * m^2) else 0)
...
/-- **The deep result:** The regulated Fourier integral converges to the 3D Bessel form as α → 0⁺.
  In this branch, the limit is the explicit `besselKhalf` expression for the free covariance.

    The proof involves:
    1. Reducing to a 1D integral by exploiting rotational symmetry
  2. Computing the Gaussian Fourier transform in spacetime dimension 3
  3. Using the Schwinger-to-`besselKhalf` identity
...
lemma aestronglyMeasurable_freeCovariance (m : ℝ) [Fact (0 < m)] :
  ...
  have hcont : ContinuousOn (fun p : SpaceTime × SpaceTime => (freeCovariance m p.1 p.2 : ℂ)) S := by
    ...
    have h_eq : Set.EqOn (fun p => freeCovariance m p.1 p.2)
        (fun p =>
          (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
            (2 * (2 * m / ‖p.1 - p.2‖) ^ (1 / 2 : ℝ) * besselKhalf (m * ‖p.1 - p.2‖))) S := by
      intro p hp
...
private lemma aestronglyMeasurable_freeCovarianceKernel (m : ℝ) [Fact (0 < m)] :
  ...
  let S : Set SpaceTime := ({0} : Set SpaceTime)ᶜ
  have hS_meas : MeasurableSet S := measurableSet_singleton 0 |>.compl
  have h_lift : ∀ {f : SpaceTime → ℝ} {s : Set SpaceTime},
      MeasurableSet s → volume sᶜ = 0 →
  ...
  have h_cont : ContinuousOn (fun z : SpaceTime => freeCovarianceKernel m z) S := by
    ...
    have h_comp : ContinuousOn
      ...
      exact norm_pos_iff.mpr hz
    exact h_comp.congr h_eq
  exact h_cont.aestronglyMeasurable hS_meas

private lemma exp_decay_implies_polynomial_decay_local
    (m α : ℝ) (hm : 0 < m) (hα : 0 < α) :
    ∃ C : ℝ, C > 0 ∧ ∀ x : ℝ, 0 ≤ x → Real.exp (-m * x) ≤ C * (1 + x) ^ (-α) := by
  ...
  · intro x hx
    ...
    have h_rpow_cancel : (1 + x) ^ α * (1 + x) ^ (-α) = 1 := by
      rw [← Real.rpow_add h_one_plus_pos]
      simp
    calc
      Real.exp (-m * x)
      ...
      _ ≤ ((α / m) ^ α * Real.exp m + 1) * (1 + x) ^ (-α) := by
            have h_nonneg : 0 ≤ (1 + x) ^ (-α) := Real.rpow_nonneg (by positivity) _
            nlinarith

private lemma freeCovarianceKernel_small_radius_bound
    (m : ℝ) (hm : 0 < m) (z : SpaceTime) (hz : z ≠ 0) (hsmall : m * ‖z‖ ≤ 1) :
    |freeCovarianceKernel m z| ≤
      ((1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
        ((2 * (2 * m) ^ (1 / 2 : ℝ) * (Real.cosh (1 / 2 : ℝ) + 2) / m) *
          (1 / m) ^ (1 / 2 : ℝ))) *
        ‖z‖ ^ (-2 : ℝ) := by
  ...
  have hr_pos : 0 < r := by
    rw [hr_def]
    exact norm_pos_iff.mpr hz
  have hmr_pos : 0 < m * r := mul_pos hm hr_pos
  have h_formula :
      freeCovarianceKernel m z =
        (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
          (2 * (2 * m / r) ^ (1 / 2 : ℝ) * besselKhalf (m * r)) := by
    have hnorm_ne : ‖(0 : SpaceTime) - z‖ ≠ 0 := by
      simpa using (norm_ne_zero_iff.mpr (neg_ne_zero.mpr hz))
    unfold freeCovarianceKernel freeCovariance freeCovarianceBessel
    rw [if_neg hnorm_ne]
    simp [hr_def, norm_neg]
  have h_bessel_bound :
      besselKhalf (m * r) ≤ (Real.cosh (1 / 2 : ℝ) + 2) / (m * r) :=
    besselKhalf_near_origin_bound (m * r) hmr_pos hsmall
  have h_const_nonneg : 0 ≤ 1 / ((4 * Real.pi) ^ (3 / 2 : ℝ)) := by
    apply one_div_nonneg.mpr
    positivity
  have h_prefactor_nonneg : 0 ≤ 2 * (2 * m / r) ^ (1 / 2 : ℝ) := by
    have : 0 ≤ (2 * m / r) ^ (1 / 2 : ℝ) := Real.rpow_nonneg (by positivity) _
    nlinarith
  have h_kernel_nonneg : 0 ≤ freeCovarianceKernel m z := by
    rw [h_formula]
    apply mul_nonneg h_const_nonneg
    apply mul_nonneg h_prefactor_nonneg
    exact (besselKhalf_pos (m * r) hmr_pos).le
  have h_div_rpow : (2 * m / r) ^ (1 / 2 : ℝ) = (2 * m) ^ (1 / 2 : ℝ) * r ^ (-(1 / 2 : ℝ)) := by
    rw [show 2 * m / r = (2 * m) / r by ring]
    rw [Real.div_rpow (by positivity : 0 ≤ 2 * m) hr_pos.le (1 / 2 : ℝ)]
    rw [Real.rpow_neg hr_pos.le]
    ring
  have h_rpow_mul : r ^ (-(1 / 2 : ℝ)) * (1 / r) = r ^ (-(3 / 2 : ℝ)) := by
    rw [show (1 / r : ℝ) = r⁻¹ by simp [one_div]]
    rw [show (r⁻¹ : ℝ) = r ^ (-1 : ℝ) by rw [Real.rpow_neg hr_pos.le, Real.rpow_one]]
    rw [← Real.rpow_add hr_pos]
    norm_num
  have h_r_le : r ≤ 1 / m := by
    rw [le_div_iff₀ hm]
    nlinarith
  have h_half_le : r ^ (1 / 2 : ℝ) ≤ (1 / m) ^ (1 / 2 : ℝ) := by
    exact Real.rpow_le_rpow hr_pos.le h_r_le (by norm_num : 0 ≤ (1 / 2 : ℝ))
  have h_rpow_decay : r ^ (-(3 / 2 : ℝ)) ≤ (1 / m) ^ (1 / 2 : ℝ) * r ^ (-2 : ℝ) := by
    have h_nonneg : 0 ≤ r ^ (-2 : ℝ) := Real.rpow_nonneg hr_pos.le _
    calc
      r ^ (-(3 / 2 : ℝ)) = r ^ (1 / 2 : ℝ) * r ^ (-2 : ℝ) := by
        rw [← Real.rpow_add hr_pos]
        norm_num
      _ ≤ (1 / m) ^ (1 / 2 : ℝ) * r ^ (-2 : ℝ) := by
        gcongr
  rw [abs_of_nonneg h_kernel_nonneg, h_formula]
  calc
    (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
        (2 * (2 * m / r) ^ (1 / 2 : ℝ) * besselKhalf (m * r))
        ≤ (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
            (2 * (2 * m / r) ^ (1 / 2 : ℝ) * ((Real.cosh (1 / 2 : ℝ) + 2) / (m * r))) := by
          apply mul_le_mul_of_nonneg_left
          · apply mul_le_mul_of_nonneg_left h_bessel_bound h_prefactor_nonneg
          · exact h_const_nonneg
    _ = (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
          ((2 * (2 * m) ^ (1 / 2 : ℝ) * (Real.cosh (1 / 2 : ℝ) + 2) / m) * r ^ (-(3 / 2 : ℝ))) := by
          rw [h_div_rpow]
          rw [show ((Real.cosh (1 / 2 : ℝ) + 2) / (m * r)) = ((Real.cosh (1 / 2 : ℝ) + 2) / m) * (1 / r) by
            field_simp [ne_of_gt hm, ne_of_gt hr_pos]]
          calc
            (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
                (2 * ((2 * m) ^ (1 / 2 : ℝ) * r ^ (-(1 / 2 : ℝ))) *
                  (((Real.cosh (1 / 2 : ℝ) + 2) / m) * (1 / r)))
                = (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
                    ((2 * (2 * m) ^ (1 / 2 : ℝ) * (Real.cosh (1 / 2 : ℝ) + 2) / m) *
                      (r ^ (-(1 / 2 : ℝ)) * (1 / r))) := by ring
            _ = (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
                  ((2 * (2 * m) ^ (1 / 2 : ℝ) * (Real.cosh (1 / 2 : ℝ) + 2) / m) *
                    r ^ (-(3 / 2 : ℝ))) := by rw [h_rpow_mul]
    _ ≤ (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
          ((2 * (2 * m) ^ (1 / 2 : ℝ) * (Real.cosh (1 / 2 : ℝ) + 2) / m) *
            ((1 / m) ^ (1 / 2 : ℝ) * r ^ (-2 : ℝ))) := by
          apply mul_le_mul_of_nonneg_left
          · apply mul_le_mul_of_nonneg_left h_rpow_decay
            positivity
          · exact h_const_nonneg
    _ = ((1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
          ((2 * (2 * m) ^ (1 / 2 : ℝ) * (Real.cosh (1 / 2 : ℝ) + 2) / m) *
            (1 / m) ^ (1 / 2 : ℝ))) * r ^ (-2 : ℝ) := by ring
    _ = ((1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
          ((2 * (2 * m) ^ (1 / 2 : ℝ) * (Real.cosh (1 / 2 : ℝ) + 2) / m) *
            (1 / m) ^ (1 / 2 : ℝ))) * ‖z‖ ^ (-2 : ℝ) := by
          rw [hr_def]

private lemma freeCovariance_exponential_bound_aux (m : ℝ) (hm : 0 < m) (u v : SpaceTime)
    (h_sep : 1 ≤ m * ‖u - v‖) :
    |freeCovariance m u v| ≤
      ((1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
        (2 * (2 * m^2) ^ (1 / 2 : ℝ)) *
        (Real.sinh 1 + 2)) * Real.exp (-m * ‖u - v‖) := by
  have huv : u ≠ v := by
    intro huv
    simp [huv] at h_sep
    linarith
  set r : ℝ := ‖u - v‖ with hr_def
  have hmr_ge1 : 1 ≤ m * r := by
    simpa [hr_def] using h_sep
  have hr_pos : 0 < r := by
    rw [hr_def]
    exact norm_pos_iff.mpr (sub_ne_zero.mpr huv)
  have h_formula :
      freeCovariance m u v =
        (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
          (2 * (2 * m / r) ^ (1 / 2 : ℝ) * besselKhalf (m * r)) := by
    have h_norm_ne : ‖u - v‖ ≠ 0 := norm_ne_zero_iff.mpr (sub_ne_zero.mpr huv)
    unfold freeCovariance freeCovarianceBessel
  ...
  have h_sinh_half_le :
    ...
    have h_two_mul : Real.sinh 1 = 2 * Real.sinh (1 / 2 : ℝ) * Real.cosh (1 / 2 : ℝ) := by
      simpa using (Real.sinh_two_mul (1 / 2 : ℝ))
  ...
  have h_bessel_bound :
    ...
    calc
      ...
      _ ≤ (Real.sinh 1 + 2) * Real.exp (-(m * r)) := by
            ...
            exact Real.exp_nonneg _
  have h_prefactor : 2 * (2 * m / r) ^ (1 / 2 : ℝ) ≤ 2 * (2 * m^2) ^ (1 / 2 : ℝ) := by
    have h_div : m / r ≤ m^2 := by
      rw [div_le_iff₀ hr_pos]
      nlinarith [hm, hmr_ge1]
    have h_base : 2 * m / r ≤ 2 * m^2 := by
      have h_eq : 2 * m / r = 2 * (m / r) := by ring
      rw [h_eq]
      nlinarith [h_div]
    have h_rpow := Real.rpow_le_rpow (by positivity : 0 ≤ 2 * m / r) h_base (by norm_num : 0 ≤ (1 / 2 : ℝ))
    linarith
  have h_const_nonneg : 0 ≤ 1 / ((4 * Real.pi) ^ (3 / 2 : ℝ)) := by
    apply one_div_nonneg.mpr
    positivity
  have h_prefactor_nonneg : 0 ≤ 2 * (2 * m / r) ^ (1 / 2 : ℝ) := by
    have h_rpow_nonneg : 0 ≤ (2 * m / r) ^ (1 / 2 : ℝ) := Real.rpow_nonneg (by positivity) _
    nlinarith
  have h_kernel_nonneg : 0 ≤ freeCovariance m u v := by
    rw [h_formula]
    apply mul_nonneg h_const_nonneg
    apply mul_nonneg h_prefactor_nonneg
    exact (besselKhalf_pos (m * r) (by positivity)).le
  rw [abs_of_nonneg h_kernel_nonneg, h_formula]
  calc
    (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
        (2 * (2 * m / r) ^ (1 / 2 : ℝ) * besselKhalf (m * r))
    ...
    _ ≤ (1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
          (2 * (2 * m^2) ^ (1 / 2 : ℝ) *
            ((Real.sinh 1 + 2) * Real.exp (-(m * r)))) := by
          apply mul_le_mul_of_nonneg_left
          · apply mul_le_mul_of_nonneg_right h_prefactor
            positivity
          · exact h_const_nonneg
    _ = ((1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
          (2 * (2 * m^2) ^ (1 / 2 : ℝ)) *
          (Real.sinh 1 + 2)) * Real.exp (-(m * r)) := by ring
    _ = ((1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
          (2 * (2 * m^2) ^ (1 / 2 : ℝ)) *
          (Real.sinh 1 + 2)) * Real.exp (-m * ‖u - v‖) := by
          rw [hr_def]
...
/-- **Polynomial decay bound for the free covariance kernel.**
  ...
  In the 3D branch this should be derived from the explicit `besselKhalf`
  representation together with near-origin and large-argument bounds.

  The bound is used later for OS1-style local integrability estimates. -/
lemma freeCovarianceKernel_decay_bound (m : ℝ) (hm : 0 < m) :
  ...
  obtain ⟨Cpoly, hCpoly_pos, hCpoly⟩ :=
    exp_decay_implies_polynomial_decay_local m 2 hm (by norm_num)
  refine ⟨max Csmall (Cexp * Cpoly), by positivity, ?_⟩
  intro z
  by_cases hz : z = 0
  · subst z
    have h_nonneg : 0 ≤ max Csmall (Cexp * Cpoly) * ‖(0 : SpaceTime)‖ ^ (-2 : ℝ) := by positivity
    simpa [freeCovarianceKernel, freeCovariance, freeCovarianceBessel] using h_nonneg
  · set r : ℝ := ‖z‖ with hr_def
    have hr_pos : 0 < r := by
      rw [hr_def]
      exact norm_pos_iff.mpr hz
    by_cases hsmall : m * r ≤ 1
    · have h_small := freeCovarianceKernel_small_radius_bound m hm z hz (by simpa [hr_def] using hsmall)
      calc
        |freeCovarianceKernel m z| ≤ Csmall * ‖z‖ ^ (-2 : ℝ) := by
              simpa [Csmall] using h_small
        _ ≤ max Csmall (Cexp * Cpoly) * ‖z‖ ^ (-2 : ℝ) := by
              gcongr
              exact le_max_left _ _
    · have hlarge : 1 ≤ m * r := by linarith
      have h_exp := freeCovariance_exponential_bound_aux m hm 0 z (by simpa [hr_def] using hlarge)
      have h_poly := hCpoly r hr_pos.le
      have h_one_add : (1 + r) ^ (-(2 : ℝ)) ≤ r ^ (-(2 : ℝ)) := by
        have h_le : r ^ (2 : ℝ) ≤ (1 + r) ^ (2 : ℝ) := by
          apply Real.rpow_le_rpow hr_pos.le
          · linarith
          · norm_num
        rw [Real.rpow_neg (by positivity : 0 ≤ 1 + r), Real.rpow_neg hr_pos.le]
        simpa [one_div] using one_div_le_one_div_of_le (Real.rpow_pos_of_pos hr_pos _) h_le
      have hCexp_nonneg : 0 ≤ Cexp := by
        unfold Cexp
        positivity
      calc
        |freeCovarianceKernel m z| = |freeCovariance m 0 z| := rfl
        _ ≤ Cexp * Real.exp (-m * r) := by
              simpa [Cexp, freeCovarianceKernel, hr_def, sub_eq_add_neg] using h_exp
        _ ≤ Cexp * (Cpoly * (1 + r) ^ (-(2 : ℝ))) := by
              gcongr
        _ = (Cexp * Cpoly) * (1 + r) ^ (-(2 : ℝ)) := by ring
        _ ≤ (Cexp * Cpoly) * r ^ (-(2 : ℝ)) := by
              gcongr
        _ ≤ max Csmall (Cexp * Cpoly) * r ^ (-(2 : ℝ)) := by
              gcongr
              exact le_max_right _ _
        _ = max Csmall (Cexp * Cpoly) * ‖z‖ ^ (-2 : ℝ) := by rw [hr_def]

/-- **Exponential decay bound for the free covariance.**

    This is the large-separation decay estimate for the 3D covariance kernel.
...
lemma freeCovariance_exponential_bound (m : ℝ) (hm : 0 < m) (u v : SpaceTime)
    ...
    |freeCovariance m u v| ≤
      ((1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
        (2 * (2 * m^2) ^ (1 / 2 : ℝ)) *
        (Real.sinh 1 + 2)) * Real.exp (-m * ‖u - v‖) := by
  exact freeCovariance_exponential_bound_aux m hm u v h_sep
...
lemma freeCovariance_exponential_bound' (m : ℝ) [Fact (0 < m)] (u v : SpaceTime)
    ...
    |freeCovariance m u v| ≤
      ((1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
        (2 * (2 * m^2) ^ (1 / 2 : ℝ)) *
        (Real.sinh 1 + 2)) * Real.exp (-m * ‖u - v‖) :=
  freeCovariance_exponential_bound m Fact.out u v h_sep

/-- Target L¹ integrability statement for the 3D Bessel covariance kernel.

  The proof splits space into a small ball, where the kernel is controlled by the
  polynomial `‖z‖⁻²` bound, and the complement, where exponential decay is upgraded
  to an integrable `(1 + ‖z‖)⁻⁴` tail. -/
lemma freeCovarianceKernel_integrable (m : ℝ) (hm : 0 < m) :
    Integrable (freeCovarianceKernel m) volume := by
  haveI : Fact (0 < m) := ⟨hm⟩
  have h_meas : AEStronglyMeasurable (fun z : SpaceTime => freeCovarianceKernel m z) volume :=
    aestronglyMeasurable_freeCovarianceKernel m
  obtain ⟨Cdecay, hCdecay_pos, hdecay⟩ := freeCovarianceKernel_decay_bound m hm
  have h_ball : IntegrableOn (freeCovarianceKernel m) (Metric.ball 0 (1 / m)) volume := by
    refine integrableOn_ball_of_rpow_decay (d := STDimension) (by norm_num [STDimension])
      (α := (2 : ℝ)) hCdecay_pos ?_ (by positivity) ?_ h_meas
  ...
  let Cexp : ℝ :=
    ((1 / ((4 * Real.pi) ^ (3 / 2 : ℝ))) *
      ...
      (Real.sinh 1 + 2))
  obtain ⟨Cpoly, hCpoly_pos, hCpoly⟩ :=
    exp_decay_implies_polynomial_decay_local m 4 hm (by norm_num)
  have hdim_lt : (Module.finrank ℝ SpaceTime : ℝ) < 4 := by
    simpa [SpaceTime, STDimension] using (show (3 : ℝ) < 4 by norm_num)
  have h_dom : Integrable (fun z : SpaceTime => (Cexp * Cpoly) * (1 + ‖z‖) ^ (-4 : ℝ)) volume := by
    exact (integrable_one_add_norm (E := SpaceTime) (μ := volume) (r := 4) hdim_lt).const_mul (Cexp * Cpoly)
  have h_tail : IntegrableOn (freeCovarianceKernel m) (Metric.ball 0 (1 / m))ᶜ volume := by
    apply Integrable.mono' h_dom.integrableOn h_meas.restrict
    rw [ae_restrict_iff' measurableSet_ball.compl]
    filter_upwards with z hz
    have h_sep : 1 ≤ m * ‖z‖ := by
      simp only [Set.mem_compl_iff, Metric.mem_ball, dist_zero_right, not_lt] at hz
      have hmul : m * (1 / m) ≤ m * ‖z‖ := mul_le_mul_of_nonneg_left hz hm.le
      simpa [hm.ne'] using hmul
    have h_exp := freeCovariance_exponential_bound m hm 0 z (by simpa using h_sep)
    have h_poly := hCpoly ‖z‖ (norm_nonneg z)
    have hCexp_nonneg : 0 ≤ Cexp := by
      unfold Cexp
      positivity
    calc
      ‖freeCovarianceKernel m z‖ = |freeCovarianceKernel m z| := by rw [Real.norm_eq_abs]
      _ = |freeCovariance m 0 z| := rfl
      _ ≤ Cexp * Real.exp (-m * ‖z‖) := by
            simpa [Cexp, freeCovarianceKernel, sub_eq_add_neg] using h_exp
      _ ≤ Cexp * (Cpoly * (1 + ‖z‖) ^ (-4 : ℝ)) := by
            gcongr
