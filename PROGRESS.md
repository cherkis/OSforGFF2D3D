...
**Branch `chore/comment-cleanup` (2026-06-09, off `main`, not pushed):** committed (a) stale-comment cleanup; (b) draft-cruft cleanup (deleted the unused "Fourier Analysis Infrastructure" block in `Covariance/Position.lean`); (c) the three diff-vs-4D docs; (d) `push_neg`→`push Not`; (e) all remaining in-repo deprecations + linter warnings. Build green (3788 jobs), **zero in-repo warnings**. Commits: `7c963fa`, `136c18d`, `c0964af`. See the latest session-log entries.
...
- 2026-06-09 — **Deprecation + linter-warning cleanup (committed on `chore/comment-cleanup`).** Cleared every in-repo build warning; the only warnings left are in the pinned dependency (`OSforGFF/*`, `Minlos/*`, `Bochner/*`, etc.) which we don't control. Verified by removing only our module oleans (`.lake/build/lib/lean/OSforGFFin{3D,2D}`) and full-recompiling our ~70 modules (deps untouched) to get the authoritative warning list, fixing, then rebuilding clean (3788 jobs).
  - `push_neg`→`push Not` (22 sites; commit `136c18d`) — deprecated in Lean v4.29.0.
  - Deprecated-lemma renames (drop-in aliases): `EuclideanSpace.single_apply`→`PiLp.single_apply`, `EuclideanSpace.norm_single`→`PiLp.norm_single`, `continuous_mul_left`→`continuous_const_mul`, `integral_union_ae`→`setIntegral_union₀`, `integral_image_eq_integral_deriv_smul_of_antitone`→`…of_antitoneOn`.
  - Linter warnings (semantics-preserving, linter-confirmed-unused via `lean_diagnostic_messages`): `DiscreteSymmetry` `ext i j`→`ext i` + drop `Matrix.one_apply`; `TimeTranslation` drop unused simp args (`Pi.single`, `Function.update`, `eq_rec_constant`, `dite_eq_ite`) + unused `h2`; `ProdIntegrable` drop unused `PiLp.single_apply`/`h`; `OS1_Regularity` (2D) drop duplicate `norm_zero` + unused `zero_mul`. Commit `c0964af`.
- 2026-06-09 — **Documentation factual audit (`docs/`).** Verified doc claims against the pinned dependency (`60ab679`) via `grep` + `#print axioms` (`lean_verify`). Found and fixed two false claims; left verified-correct content intact.
  - **No assumed axioms.** `architecture.md` and `foundational_definitions.md` claimed three assumed axioms (`schwartz_nuclear`, `minlos_theorem`, `differentiable_analyticAt_finDim`). Reality: `schwartz_nuclear`/`differentiable_analyticAt_finDim` don't exist; `minlos_theorem` is a proven `theorem` (BochnerMinlos). `#print axioms gaussianFreeField_satisfies_all_OS_axioms` returns only `propext, Classical.choice, Quot.sound` in **both** dimensions — no project-specific axioms. (Genuine `axiom`s exist only in unrelated `GaussianField/Cylinder/*` and `BochnerMinlos/Test/*`, not on the proof path.)
  - **`NonTrivial.lean` is NOT new.** `architecture.md` §4 / table, `foundational_definitions.md`, and `definitions_entering_OS_axioms.md` claimed it has "no analog in 4D" and that 4D "does not prove" non-triviality / UV divergence. Reality: the 4D library (same commit, also the local reference) has a fully-proven `OS/NonTrivial.lean` (369 lines, 0 sorries) incl. `freeCovariance_tendsto_atTop`. Ours are copied, dimension-adapted versions (consistent with `dimension_dependence_*.md`, which already list it as a copy). The dimension enters only via the Bessel input to the UV-divergence proof.
  - **Count-table fixes** (`architecture.md`): Spacetime 8→9 (3D/2D; `ProdIntegrable.lean` lives there), 4D OS 12→13. Totals (47/35/36) unchanged.
  - Verified correct (no change): Bessel apparatus genuinely new in 3D/2D (4D has only `besselK1`); the 2D extra `BesselK0Proofs.lean`; all line-number tables in `foundational_definitions.md`; closed-form covariance / heat-kernel / Plancherel constants; `freePropagatorMomentum_mathlib = 1/((2π)²‖k‖²+m²)`; master conclusion `SatisfiesAllOS (μ_GFF m)`; all OS-axiom-statement names; every identifier in the "unchanged from 4D" lists.
